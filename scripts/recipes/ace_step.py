...
#
# Usage:
#   python scripts/recipes/ace_step.py \
...
def convert_ace_step_weights(checkpoint_dir, dit_config, output_dir, dtype_str="bf16"):
    ...
    for key, value in state_dict.items():
        if key.startswith("decoder."):
            transformer_sd[key.removeprefix("decoder.")] = value
        elif key.startswith("encoder.") or key == "null_condition_emb":
            condition_encoder_sd[key] = value
        elif key.startswith("tokenizer."):
            audio_tokenizer_sd[key.removeprefix("tokenizer.")] = value
        elif key.startswith("detokenizer."):
            audio_token_detokenizer_sd[key.removeprefix("detokenizer.")] = value
        else:
            other_sd[key] = value
    ...
    if os.path.exists(silence_latent_src):
        ...
        condition_encoder_sd["silence_latent"] = silence_raw
    ...
    transformer.load_state_dict(
        get_conversion("AceStepTransformer1DModel", dict(transformer.config)).to_diffusers(transformer_sd), strict=True
    )
    ...
    condition_encoder.load_state_dict(
        get_conversion("AceStepConditionEncoder", dict(condition_encoder.config)).to_diffusers(condition_encoder_sd),
        strict=True,
    )

    print("Constructing audio_tokenizer ...")
    audio_tokenizer = AceStepAudioTokenizer(**audio_tokenizer_init_kwargs).to(target_dtype)
    audio_tokenizer.load_state_dict(
        get_conversion("AceStepAudioTokenizer", dict(audio_tokenizer.config)).to_diffusers(audio_tokenizer_sd),
        strict=True,
    )
    ...
    audio_token_detokenizer.load_state_dict(
        get_conversion("AceStepAudioTokenDetokenizer", dict(audio_token_detokenizer.config)).to_diffusers(
            audio_token_detokenizer_sd
        ),
        strict=True,
    )
