...
from diffusers.loaders.conversion.configs.prx import VARIANTS, build_config, create_scheduler_config
...
def create_transformer_from_checkpoint(checkpoint_path, config):
    state = load_denoiser_state_dict(checkpoint_path)
    dtype = next(iter(state.values())).dtype
    return PRXTransformer2DModel.from_single_file(state, config=config, torch_dtype=dtype)
...
def main(args):
    ...
    # ---- scheduler ----
    scheduler_path = os.path.join(args.output_path, "scheduler")
    os.makedirs(scheduler_path, exist_ok=True)
    with open(os.path.join(scheduler_path, "scheduler_config.json"), "w") as handle:
        json.dump(create_scheduler_config(args.shift), handle, indent=2)
