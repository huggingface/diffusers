...
$ python ./scripts/recipes/vq_diffusion.py \
...
def transformer_original_checkpoint_to_diffusers_checkpoint(model, checkpoint):
    state = {key: value for key, value in checkpoint.items() if key.startswith("transformer.transformer.")}
    return get_conversion("Transformer2DModel", dict(model.config)).to_diffusers(state)
...
if __name__ == "__main__":
    ...
    source_embeddings = learned_classifier_free_sampling_embeddings_embeddings
    if source_embeddings is None:
        source_embeddings = torch.empty(0)
    converted = get_conversion(
        "LearnedClassifierFreeSamplingEmbeddings", dict(learned_classifier_free_sampling_embeddings_model.config)
    ).to_diffusers({"transformer.empty_text_embed": source_embeddings})
    learned_classifier_free_sampling_embeddings_model.load_state_dict(converted, strict=True, assign=True)
