...
from diffusers.loaders.conversion.configs.rae import DECODER_CONFIGS
...
def _load_hf_encoder_state_dict(encoder_type: str, encoder_name_or_path: str) -> dict[str, Any]:
    ...
    if encoder_type == "dinov2":
        ...
        prefix = "" if int(transformers.__version__.split(".")[0]) >= 5 else "vision_model."
        sd = {prefix + key: value for key, value in hf_model.state_dict().items()}
        return _strip_final_layernorm_affine(sd, prefix=prefix + "post_layernorm.")
...
def convert(args: argparse.Namespace) -> None:
    ...
    state = {f"encoder.{key}": value for key, value in encoder_state_dict.items()}
    state.update({f"decoder.{key}": value for key, value in decoder_state_dict.items()})
    state["processor.image_mean"] = torch.tensor(encoder_norm_mean, dtype=torch.float32)
    state["processor.image_std"] = torch.tensor(encoder_norm_std, dtype=torch.float32)
    state["stats.latents_mean"] = torch.as_tensor(latents_mean) if latents_mean is not None else torch.zeros(1)
    state["stats.latents_std"] = torch.as_tensor(latents_std) if latents_std is not None else torch.ones(1)
    converted = convert_component_checkpoint(state, dict(model.config), "AutoencoderRAE")
