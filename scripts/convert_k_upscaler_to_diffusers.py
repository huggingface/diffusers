...
def self_attn_to_diffusers_checkpoint(checkpoint, *, diffusers_attention_prefix, attention_prefix):
    ...
    rv = {
        ...
        # to_q
        f'{diffusers_attention_prefix}.attn1.to_q.weight': weight_q,
        f'{diffusers_attention_prefix}.attn1.to_q.bias': bias_q,
        # to_k
        f'{diffusers_attention_prefix}.attn1.to_k.weight': weight_k,
        f'{diffusers_attention_prefix}.attn1.to_k.bias': bias_k,
        # to_v
        f'{diffusers_attention_prefix}.attn1.to_v.weight': weight_v,
...
def cross_attn_to_diffusers_checkpoint(checkpoint, *, diffusers_attention_prefix, diffusers_attention_index, attention_prefix):
    ...
    rv = {
      #norm2 (ada groupnorm)
     f'{diffusers_attention_prefix}.norm{diffusers_attention_index}.linear.weight': checkpoint[f"{attention_prefix}.norm_dec.mapper.weight"],
     f'{diffusers_attention_prefix}.norm{diffusers_attention_index}.linear.bias': checkpoint[f"{attention_prefix}.norm_dec.mapper.bias"],   
      # layernorm on encoder_hidden_state
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.norm_cross.weight': checkpoint[f"{attention_prefix}.norm_enc.weight"],
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.norm_cross.bias': checkpoint[f"{attention_prefix}.norm_enc.bias"],
     # to_q
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_q.weight': checkpoint[f"{attention_prefix}.q_proj.weight"],
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_q.bias': checkpoint[f"{attention_prefix}.q_proj.bias"],
     # to_k
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_k.weight': weight_k,
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_k.bias': bias_k,
     # to_v
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_v.weight': weight_v,
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_v.bias': bias_v,
     # to_out
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_out.0.weight':  checkpoint[f"{attention_prefix}.out_proj.weight"],
     f'{diffusers_attention_prefix}.attn{diffusers_attention_index}.to_out.0.bias':  checkpoint[f"{attention_prefix}.out_proj.bias"],
...
def block_to_diffusers_checkpoint(block, checkpoint, block_idx, block_type):
    ...
    if hasattr(block, "attentions"):
        for attention_idx, attention in enumerate(block.attentions):
            ...
            idx = n * attention_idx + cross_attention_index  if block_type == 'up' else n * attention_idx + cross_attention_index +1
...
def unet_model_from_original_config(original_config):
    ...
    assert len(set(original_config['depths']))==1, "UNet2DConditionModel currently do not support blocks with different number of layers"
...
def main(args):
    ...
    orig_weights_path = huggingface_hub.hf_hub_download(UPSCALER_REPO, "laion_text_cond_latent_upscaler_2_1_00470000_slim.pth")
