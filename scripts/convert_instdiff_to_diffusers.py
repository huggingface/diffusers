...
def convert_instdiff_vae_checkpoint(checkpoint, config):
...
def convert_instdiff_unet_checkpoint(checkpoint, config, path=None, extract_ema=False):
    unet_state_dict = {}
    checkpoint = checkpoint["ema" if extract_ema else "model"]
    keys = list(checkpoint.keys())
...
def create_unet_config(original_config, image_size: int):
    ...
    config = {
        ...
        "attention_type": "unifusion",
    }

    return config


def convert_instdiff_to_diffusers(
    checkpoint_path: str,
    ...
    converted_unet_checkpoint = convert_instdiff_unet_checkpoint(
    ...
    converted_vae_checkpoint = convert_instdiff_vae_checkpoint(checkpoint, vae_config)
    ...
    pipe = StableDiffusionINSTDIFFPipeline(
        vae=vae,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="The YAML config file corresponding to the InstanceDiffusion architecture.",
    )
    parser.add_argument(
        "--extract_ema",
        action="store_true",
    ...
    pipe = convert_instdiff_to_diffusers(
