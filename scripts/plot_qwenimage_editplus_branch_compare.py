...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot QwenImageEditPlus timing comparison.")
    ...
    parser.add_argument("--output", default="qwenimage_editplus_branch_compare.png")
...
def main() -> None:
    ...
    main_steps = main_data.get("steps") or 1
    branch_steps = branch_data.get("steps") or 1
    means = [main_data["mean_seconds"] / main_steps, branch_data["mean_seconds"] / branch_steps]
    stds = [main_data["std_seconds"] / main_steps, branch_data["std_seconds"] / branch_steps]

    fig, ax = plt.subplots(figsize=(6, 4))
    bars = ax.bar(labels, means, yerr=stds, color=["#4E79A7", "#F28E2B"], edgecolor="black")
    ax.set_ylabel("Time per step (seconds)")
    ax.set_title("QwenImageEditPlus single-step runtime (lower is better)")
