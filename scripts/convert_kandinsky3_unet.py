...
from your_diffusers_module import UNetKandi3, YourConfigClass  # Replace with your actual import

def convert_state_dict(unet_model, unet_kandi3_model):
    """
    Convert the state dict of a U-Net model to match the key format expected by UNetKandi3 model.
    Args:
        unet_model (torch.nn.Module): The original U-Net model.
        unet_kandi3_model (torch.nn.Module): The UNetKandi3 model to match keys with.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert U-Net PyTorch model to UNetKandi3 format")
