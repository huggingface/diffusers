...
def convert_ncsnpp_checkpoint(checkpoint, config):
    ...
    for i, block in enumerate(new_model_architecture.upsample_blocks):
        ...
        if hasattr(block, "attentions") and block.attentions is not None:
            block.attentions[0].set_weight(new_model_architecture.all_modules[module_index])
            module_index += 1
        if hasattr(block, "resnet_up") and block.resnet_up is not None:
            block.skip_norm.weight.data = new_model_architecture.all_modules[module_index].weight.data
            block.skip_norm.bias.data = new_model_architecture.all_modules[module_index].bias.data
            module_index += 1
            block.skip_conv.weight.data = new_model_architecture.all_modules[module_index].weight.data
            block.skip_conv.bias.data = new_model_architecture.all_modules[module_index].bias.data
            module_index += 1
            block.resnet_up.set_weight(new_model_architecture.all_modules[module_index])
            module_index += 1

    new_model_architecture.conv_norm_out.weight.data = new_model_architecture.all_modules[module_index].weight.data
    new_model_architecture.conv_norm_out.bias.data = new_model_architecture.all_modules[module_index].bias.data
    module_index += 1
    new_model_architecture.conv_out.weight.data = new_model_architecture.all_modules[module_index].weight.data
    new_model_architecture.conv_out.bias.data = new_model_architecture.all_modules[module_index].bias.data
