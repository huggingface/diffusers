...
这引起了人们对生成式人工智能的极大兴趣，你可能已经在网上见过扩散生成的图像了。🧨 Diffusers库的目的是让大家更易上手扩散模型。
...
无论你是开发者还是普通用户，这个快速指南将向你介绍🧨 Diffusers，并帮助你快速使用和生成！该库有三个主要部分如下：

* [`DiffusionPipeline`]是一个高级的端到端类，旨在通过预训练的扩散模型快速生成样本进行推理。
* 可以作为创建扩散系统的构建模块的流行的预训练[模型](./api/models)架构和模块。
* 许多不同的[调度器](./api/schedulers/overview) - 控制如何在训练过程中添加噪声的算法，以及如何在推理过程中生成去噪图像的算法。

快速浏览将告诉你如何使用[`DiffusionPipeline`]进行推理，然后指导你如何结合模型和调度器以复制[`DiffusionPipeline`]内部发生的事情。

<Tip>

快速入门是🧨[Diffusers入门](https://colab.research.google.com/github/huggingface/notebooks/blob/main/diffusers/diffusers_intro.ipynb)的简化版，可以帮助你快速上手。如果你想了解更多关于🧨 Diffusers的目标、设计理念以及关于它的核心API的更多细节，可以点击链接查看。

</Tip>

在开始之前，确认一下你已经安装好了所有需要的库：

```bash
pip install --upgrade diffusers accelerate transformers
```

- [🤗 Accelerate](https://huggingface.co/docs/accelerate/index) 在推理和训练过程中加速模型加载。speeds up model loading for inference and training.
...
[`DiffusionPipeline`]是使用预训练的扩散系统进行推理的最简单方法。它是一个包含模型和调度器的端到端系统。你可以直接使用[DiffusionPipeline]完成许多任务。请查看下面的表格以了解一些支持的任务，要获取完整的支持任务列表，请查看[🧨 Diffusers Summary](./api/pipelines/overview#diffusers-summary) 。
...
首先创建一个[`DiffusionPipeline`]的实例，并指定要下载的管道检查点。
...
```
[DiffusionPipeline]会下载并缓存所有的建模、标记化和调度组件。你可以看到Stable Diffusion管道是由[UNet2DConditionModel]和[PNDMScheduler]等组件组成的：
...
我们强烈建议在GPU上运行这个管道，因为该模型由大约14亿个参数组成。
你可以像使用Pytorch那样把生成器对象移到GPU上：

```python
>>> pipeline.to("cuda")
```

现在你可以向`管道`传递一个文本提示来生成图像，然后获得去噪的图像。默认情况下，图像输出被放在一个[`PIL.Image`](https://pillow.readthedocs.io/en/stable/reference/Image.html?highlight=image#the-image-class)对象中。
...
试着用新的调度器生成一个图像，看看你能否注意到不同之处。

在下一节中，你将仔细观察组成[`DiffusionPipeline`]的组件——模型和调度器，并学习如何使用这些组件来生成猫咪的图像。

## 模型

大多数模型取一个噪声样本，在每个时间点预测*噪声残差*（其他模型则直接学习预测前一个样本或速度或[`v-prediction`](https://github.com/huggingface/diffusers/blob/5e5ce13e2f89ac45a0066cb3f369462a3cf1d9ef/src/diffusers/schedulers/scheduling_ddim.py#L110)），即噪声较小的图像与输入图像的差异。你可以混合和匹配模型创建其他扩散系统。

模型是用[`~ModelMixin.from_pretrained`]方法启动的，该方法还本地缓存了模型权重，所以下次加载模型时更快。对于快速入门，你加载的是[`UNet2DModel`]，这是一个基础的无条件图像生成模型，该模型有一个在猫咪图像上训练的检查点：
...
想生成实际的样本，你需要一个调度器来指导去噪过程。在下一节中，你将学习如何把模型与调度器结合起来。

## ## 调度器

调度器管理从一个噪声样本到一个噪声较小的样本的过程，给出模型输出——在这种情况下，它是`noisy_residual`。



<Tip>

🧨 Diffusers是一个用于构建扩散系统的工具箱。预定义好的扩散系统[`DiffusionPipeline`]是快速上手使用的便捷方法，你也可以单独选择自己的模型和调度器组件来建立一个自定义的扩散系统。

</Tip>

对于快速浏览，你将用它的[`~diffusers.ConfigMixin.from_config`]方法实例化[`DDPMScheduler`]：
...
要预测一个噪音稍小的图像，请将以下内容传递给调度器的[`~diffusers.DDPMScheduler.step`]方法：模型输出、`timestep`和当前`sample`。
...
* 通过[在GPU上优化PyTorch](./optimization/fp16)指南，以及运行[在Apple (M1/M2)上的Stable Diffusion](./optimization/mps)和[ONNX Runtime](./optimization/onnx)的教程，更深入地了解如何加快🧨Diffuser。