...
- VersatileDiffusion is conceptually very similar as [Stable Diffusion](./api/pipelines/stable_diffusion/overview),  but instead of providing just an image data stream conditioned on text, VersatileDiffusion provides both an image and text data stream and can be conditioned on both text and image.
