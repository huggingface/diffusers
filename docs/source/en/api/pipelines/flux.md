...
Flux can run inference in FP16 mode (i.e. to accelerate inference on Turing/Volta GPUs) but produces the different outputs compared to FP32/BF16. The issue is that some activations in the text encoders have to be clipped when running in FP16. Forcing text encoders to run with FP32 inference thus removes this output difference.
