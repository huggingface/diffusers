...
image_embeds, negative_image_embeds = pipe_prior(
    [prompt, negative_prompt], generator=[generator, generator]
).image_embeds
```
...
```py
images = pipe(
    prompt, image_embeds=image_embeds, negative_image_embeds=negative_image_embeds, negative_prompt=negative_prompt
).images
```
