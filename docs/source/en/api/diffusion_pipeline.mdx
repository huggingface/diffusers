...
<Tip>
	...
	components of diffusion pipelines are usually trained individually, so we suggest directly working 
	with [`UNetModel`] and [`UNetConditionModel`] instead.
...
detected and consequently loads each component of the 
pipeline and passes them into the `__init__` function of the pipeline.
