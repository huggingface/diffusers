...
Load a model in Diffusers format with [`~ModelMixin.from_pretrained`], and load a model in the original checkpoint format with [`~FromOriginalModelMixin.from_pretrained`].
