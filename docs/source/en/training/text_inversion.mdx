...
Now we can run the pipeline making sure that the placeholder_token `<cat-toy>` is used in our prompt.
...
To do so, you can first download the an embedding vector from [civitAI](https://civitai.com/models/3036?modelVersionId=8387)
and then load it locally:
```python
pipe.load_textual_inversion("./charturner.py")
```
</pt>
<jax>
