...
The pretrained weights of the other components from the text-to-image model are initialized from their checkpoints, but the `unet` model weights are randomly initialized. It is important to finetune the model for inpainting now because otherwise the model returns noise.
