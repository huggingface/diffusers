...
Note that we will gradually be depcrecating the use of [`UNet2DConditionLoadersMixin.load_attn_procs`] since we now have a more general
method to load the LoRA parameters -- [`LoraLoaderMixin.load_lora_weights`]. This is because
