...
This is an implementation of [SVDiff: Compact Parameter Space for Diffusion Fine-Tuning](https://arxiv.org/abs/2303.11305) by with 🧨 diffusers. To quickly get started we recommend using this [Colab Notebook](https://colab.research.google.com/gist/mkshing/8a2c025c4f20bf84b7d271e832b6f5f8/svdiff_pytorch.ipynb). 

This training example was contributed by [`mkshing`](https://github.com/mkshing) in [this PR](https://github.com/huggingface/diffusers/pull/3107). 
...
Let's first download the mini dog dataset:
...
According to the [paper](https://arxiv.org/abs/2303.11305), the learning rate for SVDiff needs to be 1000 times larger than the learning rate typically used for fine-tuning. 
...
After training, SVDiff weights can be loaded using the `set_spectral_shifts()` function and you just pass the output models into the original pipeline:
...
image = pipe("photo of a blue chair with black legs", num_inference_steps=25).images[0]
image.save("image.png")
```
...
# previous steps including pipeline loading, etc. have been omitted for brevity.
image = "path-to-image"
prompt = "photo of a blue chair with black legs"
...
You can adjust the strength of the weights. The larger the scale, the closer the generated image is to your input concept. On the other hand, decreasing that value generates images that follow the text prompt more.
