...
pipeline = StableDiffusionImg2ImgPipeline.from_pretrained("Lykon/dreamshaper-8", use_safetensors=True)
```
...
pipeline = DiffusionPipeline.from_pretrained("Lykon/dreamshaper-8", safety_checker=None, use_safetensors=True)
"""
...
- `variant` specifies which files should be loaded from the repository. For example, if you want to load a non-EMA variant of a UNet from [Lykon/dreamshaper-8](https://hf.co/Lykon/dreamshaper-8/tree/main/unet), set `variant="non_ema"` to download the `non_ema` file.
...
pipeline.save_pretrained("Lykon/dreamshaper-8", variant="fp16")
```
...
```py
pipeline.save_pretrained("Lykon/dreamshaper-8", variant="non_ema")
```
