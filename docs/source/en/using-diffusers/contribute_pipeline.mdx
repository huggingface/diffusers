...
- It can be loaded with the [`~DiffusionPipeline.from_pretrained.custom_pipeline`] argument.
- The model weights and scheduler configuration are loaded from [`~DiffusionPipeline.from_pretrained.pretrained_model_name_or_path`].
