...
Otherwise, a variant is **identical** to the original checkpoint. They have exactly the same serialization format (such as [Safetensors](./using-diffusers/using_safetensors)), model structure, and weights have identical tensor shapes.

Load a variant by specifying the `variant` argument in [`DiffusionPipeline.from_pretrained`]. 🧨 Diffusers won't download a variant unless it is explicitly specified, so you don't have to worry about downloading and caching more checkpoints than you need.
...
stable_diffusion.save_pretrained("runwayml/stable-diffusion-v1-5")
```

If you don't save the variant to an existing folder, you'll notice the weights in the new folder have `.fp16.bin` or `.non-ema.bin` appended to the end of the file name. In this case, you must specify the `variant` argument otherwise it'll throw an `Exception` because it can't find the original checkpoint:
