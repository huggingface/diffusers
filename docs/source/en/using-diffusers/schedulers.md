...
This guide will show you how to load schedulers and models to customize a pipeline. You'll use the [Lykon/dreamshaper-8](https://hf.co/Lykon/dreamshaper-8) checkpoint throughout this guide, so let's load it first.
...
ddim = DDIMScheduler.from_pretrained("Lykon/dreamshaper-8", subfolder="scheduler")
```
...
```python
pipeline = DiffusionPipeline.from_pretrained(
    "Lykon/dreamshaper-8", scheduler=ddim, torch_dtype=torch.float16, use_safetensors=True
).to("cuda")
```
...
Models can be loaded from a subfolder with the `subfolder` argument. For example, the model weights for [Lykon/dreamshaper-8](https://hf.co/Lykon/dreamshaper-8) are stored in the [unet](https://hf.co/Lykon/dreamshaper-8/tree/main/unet) subfolder.

```python
from diffusers import UNet2DConditionModel

unet = UNet2DConditionModel.from_pretrained("Lykon/dreamshaper-8", subfolder="unet", use_safetensors=True)
```
