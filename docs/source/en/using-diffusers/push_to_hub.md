...
```py
scheduler = DDIMScheduler.from_pretrained("your-namespace/my-controlnet-scheduler")
```

## Pipeline

To push a pipeline to the Hub, load it with [`~DiffusionPipeline.from_pretrained`], then call [`~diffusers.utils.PushToHubMixin.push_to_hub`] with a repository id.
