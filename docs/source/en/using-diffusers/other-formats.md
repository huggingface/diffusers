...
1. Diffusers-multifolder layout: there may be several separate safetensors files, one for each pipeline component (text encoder, UNet, VAE), organized in subfolders (check out the [Lykon/dreamshaper-8](https://hf.co/Lykon/dreamshaper-8/tree/main) repository as an example)
...
pipeline = StableDiffusionPipeline.from_single_file(
    "https://huggingface.co/Lykon/dreamshaper-8/blob/main/v1-5-pruned.ckpt"
)
```
...
ckpt_path = "https://huggingface.co/Lykon/dreamshaper-8/blob/main/v1-5-pruned-emaonly.safetensors"
