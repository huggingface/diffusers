...
💡 We strongly recommend taking a look at PyTorch's [statement about reproducibility](https://pytorch.org/docs/stable/notes/randomness.html):
...
But if you want to reproduce the same image on a CPU, you'll need to use a PyTorch [`Generator`](https://pytorch.org/docs/stable/generated/torch.randn.html) and set a seed:
...
If you run this code example on your specific hardware and version, you should get a similar if not the same, result.
...
Writing a reproducible pipeline on a GPU is a bit trickier, and full reproducibility across different hardware is not guaranteed because matrix multiplication - which diffusion pipelines require a lot of - is less deterministic on a GPU than a CPU. For example, if you run the same code example above on a CPU on a GPU: 
