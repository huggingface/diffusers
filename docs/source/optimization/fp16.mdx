...
To decode large batches of images with limited VRAM, or to enable image batches with 32 images or more, you can use sliced VAE decode that decodes the batch latents one image at a time. 
You also want to use attention slicing or xformers to run unet with large batch sizes and limited VRAM.
...
There may also be a small performance boost in VAE decode on multi-image batches.
