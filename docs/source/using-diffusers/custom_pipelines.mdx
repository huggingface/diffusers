...
All you need to do is to pass the custom pipeline repo id with the `custom_pipeline` argument along side the repo from where you wish to load the pipeline modules.
...
By loading a costum pipeline from the Hugging Face Hub, you are trusting that the code you are loading 
...
Community pipelines are summarized in the [community example folder](https://github.com/huggingface/diffusers/tree/main/examples/community) 

Similarly, you need to pass both the *repo id* from where you wish to load the weigts as well as the `custom_pipeline` argument. Here the `custom_pipeline` argument should consist simply of the filename of the community pipeline excluding the `.py` suffix, *e.g.* `clip_guided_stable_diffusion`.
...
Now you can upload this short file under the name `pipeline.py` in your prefered model repository.
Finally, we can load the custom pipeline by passing the model repository name, *e.g.* `patrickvonplaten/my_custom_pipeline` along side the model repository from where we want to load the `unet` and `scheduler` components.
