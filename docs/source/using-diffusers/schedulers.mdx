...
Diffusion pipelines are inherently a collection of diffusion models and schedulers that are partly independent for each other. This means that one is able to switch out parts of the pipeline to better customize 
...
Now we show how easy it is to change the scheduler of a pipeline. Every scheduler has a property [`ConfigMixin.compatibles`] 
