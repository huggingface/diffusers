...
- First, the scheduler is loaded with [`SchedulerMixin.from_pretrained`] since it only depends on a configuration file and not any parameterized weights
...
Schedulers cannot be loaded via a `from_pretrained` method, but instead rely on [`SchedulerMixin.from_pretrained`]. Schedulers are **not parameterized** or **trained**, but instead purely defined by a configuration file.
