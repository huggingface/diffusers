...
`num_images_per_prompt`를 설정하여 동일한 프롬프트에 대해 다른 이미지를 비교할 수도 있습니다. 다른 체크포인트([v1-5](https://huggingface.co/Lykon/dreamshaper-8))로 동일한 파이프라인을 실행하면 다음과 같은 결과가 나옵니다:
...
그런 다음 [v1-5 checkpoint](https://huggingface.co/Lykon/dreamshaper-8)를 로드하여 이미지를 생성합니다:
...
[v1-5](https://huggingface.co/Lykon/dreamshaper-8) 체크포인트가 이전 버전보다 더 나은 성능을 보이는 것 같습니다. 그러나 CLIP 점수를 계산하기 위해 사용한 프롬프트의 수가 상당히 적습니다. 보다 실용적인 평가를 위해서는 이 수를 훨씬 높게 설정하고, 프롬프트를 다양하게 사용해야 합니다.
