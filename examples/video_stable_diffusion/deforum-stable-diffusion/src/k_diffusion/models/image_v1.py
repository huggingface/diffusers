...
class ImageDenoiserModelV1(nn.Module):
    def __init__(
        ...
        self.proj_out = nn.Conv2d(
            channels[max(0, skip_stages - 1)], c_in * self.patch_size**2 + (1 if self.has_variance else 0), 1
        )
    ...
    def forward(
        ...
        mapping_cond_embed = (
            torch.zeros_like(timestep_embed) if mapping_cond is None else self.mapping_cond(mapping_cond)
        )
