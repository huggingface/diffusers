...
class SamplerCallback(object):
    ...
    def __init__(self, args, root, mask=None, init_latent=None, sigmas=None, sampler=None, verbose=False):
        ...
        self.paths_to_image_steps = [
            os.path.join(args.outdir, f"{args.timestring}_{index:02}_{args.seed}") for index in range(args.n_samples)
        ]
    ...
    def img_callback_(self, img, pred_x0, i):
        ...
        if self.mask is not None:
            ...
            init_noise = self.sampler.stochastic_encode(
                self.init_latent, torch.tensor([i_inv] * self.batch_size).to(self.device), noise=self.noise
            )
