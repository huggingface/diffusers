        ...
        with open(settings_filename, "r") as f:
            ...
            for i, k in enumerate(args_dict):
                if k in jdata:
                    ...
                    if verbose:
                        print(
                            f"key {k} doesn't exist in the custom settings data! using the default value of {args_dict[k]}"
                        )
            for i, k in enumerate(anim_args_dict):
                if k in jdata:
                    ...
                    if verbose:
                        print(
                            f"key {k} doesn't exist in the custom settings data! using the default value of {anim_args_dict[k]}"
                        )
            if verbose:
                print(args_dict)
