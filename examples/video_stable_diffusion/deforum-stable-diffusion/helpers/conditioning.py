...
def make_rgb_color_match_loss(root, target, n_colors, ignore_sat_weight=None, img_shape=None, device="cuda:0"):
    ...
    def rgb_color_ratio_loss(x, sigma, **kwargs):
        ...
        all_color_norm_distances = (
            torch.ones(len(color_list), x.shape[0], x.shape[2], x.shape[3]).to(device) * 6.0
        )  # distance to color won't be more than max norm1 distance between -1 and 1 in 3 color dimensions
        ...
        if color_distributions is None:
            color_distributions = color_distance_distributions(
                n_colors, (x.shape[2], x.shape[3]), color_list, color_counts, n_images=x.shape[0]
            )
