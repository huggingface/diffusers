...
sub_p_res = subprocess.run(
    ["nvidia-smi", "--query-gpu=name,memory.total,memory.free", "--format=csv,noheader"], stdout=subprocess.PIPE
).stdout.decode("utf-8")
print(f"{sub_p_res[:-1]}")
...
def setup_environment():
    ...
    if "google.colab" in str(ipy):
        ...
        if use_xformers_for_colab:
            ...
            v_card_name = subprocess.run(
                ["nvidia-smi", "--query-gpu=name", "--format=csv,noheader"], stdout=subprocess.PIPE
            ).stdout.decode("utf-8")
            ...
            x_link = (
                "https://github.com/TheLastBen/fast-stable-diffusion/raw/main/precompiled/"
                + name_to_download
                + "/"
                + x_ver
            )
            ...
            all_process = [
                ...
                [
                    "mv",
                    "deforum-stable-diffusion/src/ldm/modules/attention_xformers.py",
                    "deforum-stable-diffusion/src/ldm/modules/attention.py",
                ],
            ]
...
def DeforumArgs():
    ...
    colormatch_image = (
        "https://www.saasdesign.io/wp-content/uploads/2021/02/palette-3-min-980x588.png"  # @param {type:"string"}
    )
...
if skip_video_for_run_all == True:
    ...
    if make_gif:
        ...
        cmd_gif = ["ffmpeg", "-y", "-i", mp4_path, "-r", str(fps), gif_path]
