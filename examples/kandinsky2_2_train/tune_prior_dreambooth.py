...
import diffusers
from diffusers import PriorTransformer, UnCLIPScheduler, DDPMScheduler, KandinskyV22PriorPipeline
...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        self.instance_paths = [os.path.join(instance_data_dir, i) for i in os.listdir(instance_data_dir) if '.jpg' in i or '.png' in i or '.jpeg' in i]
    ...
    def __getitem__(self, i):
        example = {}
        img = Image.open(self.instance_paths[i])
        example["instance_images"] = torch.from_numpy(self.image_processor(img).pixel_values[0])
        ...
        example["instance_attention_mask"] = text_inputs.attention_mask.bool()[0]
        if self.class_dataset is not None:
            example["class_images"] = self.class_dataset[i % self.class_dataset.shape[0]]
            text_inputs = self.tokenizer(
                self.class_prompt,
                padding="max_length",
                max_length=self.tokenizer.model_max_length,
            ...
            example["class_prompt_ids"] = text_inputs.input_ids[0]
            example["class_attention_mask"] = text_inputs.attention_mask.bool()[0] 
        return example
...
def main():
    ...
    if args.with_prior_preservation:
        pipe_prior = KandinskyV22PriorPipeline.from_pretrained(args.pretrained_prior_path, torch_dtype=torch.float16).to(accelerator.device)
        class_dataset = []
        for i in tqdm(range(args.num_class_images)):
            image_emb = pipe_prior(args.class_prompt).image_embeds
            class_dataset.append(image_emb.to('cpu'))
        class_dataset = torch.cat(class_dataset, dim=0)
        del pipe_prior
    ...
    train_dataset = ImageDataset(instance_data_dir=args.instance_data_dir,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(prior):
                ...
                text_input_ids, text_mask, clip_images = batch['input_ids'].to(accelerator.device), batch['attention_mask'].to(accelerator.device), batch['pixel_values'].to(accelerator.device).to(weight_dtype)
                if args.with_prior_preservation:
                ...
                with torch.no_grad():
                    ...
                    if args.with_prior_preservation:
                        image_embeds = torch.cat((image_embeds, class_pixel_values), dim=0)
