...
better target alignment. Recommended pipeline (mirrors SpecForge's
``scripts/prepare_data.py`` → ``scripts/regenerate_train_data.py``):

  1. Standardise the dataset into JSONL Conversation Format
     (``{"id": ..., "conversations": [{"role": "user"|"assistant", "content": ...}, ...]}``).
     SpecForge ships ready-made converters for UltraChat / ShareGPT / OPC.
  ...
  3. Re-roll the assistant turns through the served target. Use
