...
Color scheme (colorblind-safe, no red/green):
  ...
  dim           = current block token still being refined (LLaDA2)
...
DFlash two-phase flash per block:
  ...
  2. `draft_pause` seconds pause
  3. Accepted tokens snap to bold yellow, rejected positions return to ░

LLaDA2 block-by-block:
  block_x grows one block at a time so only committed + active block are shown.
  Tokens being refined in the active block appear dim until they commit.
...
@contextmanager
def noop():
    """No-op context manager used when visualization is disabled."""
    yield
