...
def parse_args() -> TrainConfig:
    parser = argparse.ArgumentParser(description="Train a uniform token diffusion LM (UDLM-style) with Accelerate.")
    ...
    parser.add_argument("--output_dir", type=str, default="udlm-output")
...
def udlm_diffusion_loss(
    ...
    term1 = (V / x_bar_zt) - (V / x_bar_theta_zt)

    term2 = ((x_bar / x_bar_zt) * (x_bar_theta_zt.log() - x_bar_theta.log() + x_bar.log() - x_bar_zt.log())).sum(
        dim=-1, keepdim=True
    )
    ...
    for epoch in range(num_train_epochs):
        for batch in train_dataloader:
            with accelerator.accumulate(model):
                ...
                if scheduler.exclude_mask_from_uniform:
                    logits = logits.clone()
