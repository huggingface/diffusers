...
def parse_args():
    ...
    parser.add_argument("--sampled_timesteps", type=list, default=[0,106,200])
    parser.add_argument("--num_inference_steps", type=int, default=201)
    parser.add_argument("--target_ema_mode", type=str, default="adaptive")
    parser.add_argument("--start_ema", type=float, default=0.95)
    parser.add_argument("--scale_mode", type=str, default="progressive")
    parser.add_argument("--start_scales", type=int, default=2)
    parser.add_argument("--end_scales", type=int, default=200)
...
def main(args):
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            ...
            target_model.save_pretrained(os.path.join(output_dir, "target_unet_ema"))
        ...
        def load_model_hook(models, input_dir):
            ...
            load_model = EMAModel.from_pretrained(os.path.join(input_dir, "target_unet_ema"), UNet2DModel)
    ...
    accepts_prediction_type = "prediction_type" in set(inspect.signature(CMScheduler.__init__).parameters.keys())
    if accepts_prediction_type:
        raise NotImplementedError()
    else:
        noise_scheduler = CMScheduler(
            ...
            s_max = args.s_max,
            rho = args.rho,
            sampled_timesteps = args.sampled_timesteps,
    ...
    target_model = EMAModel(
        ...
        power=args.ema_power,
        model_cls=UNet2DModel,
        model_config=ori_model.config,
    )
    target_model.decay = target_ema
    target_model.min_decay = target_ema
    target_model.to(accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            noisy_images, timesteps, num_scales = noise_scheduler.add_noise(clean_images, global_step, bsz)
            ...
            with accelerator.accumulate(model):
                ...
                if args.schedule_sampler == "uniform":
                    ...
                    # e.g. lognormal for EDM
                    # https://github.com/openai/consistency_models/blob/main/scripts/launch.sh
                    raise NotImplementedError()
            ...
            if accelerator.sync_gradients:
                ...
                target_model.decay = target_ema
                target_model.min_decay = target_ema
                target_model.step(model.parameters())
            ...
            logs["target_mode_ema_decay"] = target_model.cur_decay_value
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                #target_model.store(unet.parameters())
                #target_model.copy_to(unet.parameters())
                if args.use_ema:
                ...
                #target_model.restore(unet.parameters())
                if args.use_ema:
