...
def log_validation(
    ...
    #autocast_ctx = torch.autocast(accelerator.device.type) if not if_lora_validation else nullcontext()
    ...
    for tracker in accelerator.trackers:
        phase_name = "test" if if_lora_validation else "validation"
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=32,
        help=("The alpha parameter for the LoRA scaling. A good rule of thumb is to set alpha to rank/2 for stable training."),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Save a checkpoint of the training state every X epochs. If specified, this will override"
            " --checkpointing_steps and save based on epochs instead."
        ),
    )
    parser.add_argument(
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # 处理repeats参数，确保instance_images_path与instance_images长度一致
        if hasattr(self, 'instance_images_path') and repeats > 1:
            original_paths = self.instance_images_path.copy()
            self.instance_images_path = []
            for path in original_paths:
                self.instance_images_path.extend(itertools.repeat(path, repeats))
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                # If checkpointing by epochs is set, skip checkpointing by steps
                if accelerator.is_main_process and args.checkpointing_epochs is None:
        ...
        # At the end of each epoch, if checkpointing by epoch is set, save a checkpoint
        if accelerator.is_main_process and args.checkpointing_epochs is not None:
            if (epoch + 1) % args.checkpointing_epochs == 0:
                # Check if the total number of checkpoints will be exceeded
                if args.checkpoints_total_limit is not None:
                    checkpoints = os.listdir(args.output_dir)
                    checkpoints = [d for d in checkpoints if d.startswith("checkpoint")]
                    checkpoints = sorted(checkpoints, key=lambda x: int(x.split("-")[1]))

                    # Ensure that before saving, there are at most `checkpoints_total_limit - 1` checkpoints
                    if len(checkpoints) >= args.checkpoints_total_limit:
                        num_to_remove = len(checkpoints) - args.checkpoints_total_limit + 1
                        removing_checkpoints = checkpoints[0:num_to_remove]
                ...
                save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                accelerator.save_state(save_path)
                logger.info(f"Saved state to {save_path} at epoch {epoch+1}")

        if accelerator.is_main_process:
            # Validation phase: first run inference with the original pipeline, then load LoRA weights on the same pipeline and run inference again
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                # 1. Create the original pipeline
                if not args.train_text_encoder:
                    text_encoder_one, text_encoder_two = load_text_encoders(text_encoder_cls_one, text_encoder_cls_two)
                    text_encoder_one.to(weight_dtype)
                    text_encoder_two.to(weight_dtype)
                pipeline: FluxFillPipeline = FluxFillPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    revision=args.revision,
                    variant=args.variant,
                ...
                print(f"[Validation][Origin] Generating results with the original model, prompt: {args.validation_prompt}")
                pipeline_args: dict[str, object] = {
                    "prompt": args.validation_prompt,
                    "image": val_image,
                    "mask_image": val_mask
                }
                # Use torch.no_grad() to wrap validation inference to avoid gradient computation
                with torch.no_grad():
                    # Run validation and generate images with the original model
                    images_origin = log_validation(
                        pipeline=pipeline,
                        args=args,
                        accelerator=accelerator,
                ...
                # Save images generated by the original model
                temp_dir: str = os.path.join(args.output_dir, "temp")
                if not os.path.exists(temp_dir):
                    os.makedirs(temp_dir, exist_ok=True)
                for idx, img in enumerate(images_origin):
                    img_save_path: str = os.path.join(temp_dir, f"validation_epoch{epoch}_img{idx}_origin.png")
                    img.save(img_save_path)
                    print(f"[Validation][Origin] Saved image generated by the original model: {img_save_path}")

                # 2. Load the current checkpoint's LoRA weights on the same pipeline
                checkpoint_dir: str = os.path.join(args.output_dir, f"checkpoint-{epoch}")
                pipeline.load_lora_weights(checkpoint_dir)

                print(f"[Validation][With LoRA] Generating results with LoRA weights loaded, prompt: {args.validation_prompt}")
                with torch.no_grad():
                    images_lora = log_validation(
                        pipeline=pipeline,
                        args=args,
                        accelerator=accelerator,
                        pipeline_args=pipeline_args,
                        epoch=epoch,
                        if_lora_validation=True
                    )
                # Save images generated after loading LoRA weights
                for idx, img in enumerate(images_lora):
                    img_save_path: str = os.path.join(temp_dir, f"validation_epoch{epoch}_img{idx}_with_lora.png")
                    img.save(img_save_path)
                    print(f"[Validation][With LoRA] Saved image generated with LoRA weights: {img_save_path}")
    ...
    if accelerator.is_main_process:
        ...
        # Final inference
        # Here, the pipeline is reloaded: first as the original pipeline, then LoRA weights are added
        pipeline: FluxFillPipeline = FluxFillPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
            revision=args.revision,
            variant=args.variant,
        ...
        # Load LoRA weights on the pipeline
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            # Save images generated with LoRA weights for comparison with the original model results
            temp_dir: str = os.path.join(args.output_dir, "temp")
            if not os.path.exists(temp_dir):
                os.makedirs(temp_dir, exist_ok=True)
