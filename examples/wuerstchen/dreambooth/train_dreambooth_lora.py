...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                images = pipeline(example["prompt"], prior_timesteps=DEFAULT_STAGE_C_TIMESTEPS).images
    ...
    elif args.pretrained_prior_model_name_or_path:
    ...
    noise_scheduler = DDPMWuerstchenScheduler.from_pretrained(
        args.pretrained_prior_model_name_or_path, subfolder="scheduler"
    )
    ...
    prior = WuerstchenPrior.from_pretrained(
        args.pretrained_prior_model_name_or_path, subfolder="prior", revision=args.revision, variant=args.variant
    )
    ...
    # Move prior, image encoder and text_encoder to device and cast to weight_dtype
    ...
    if args.train_text_encoder:
        text_lora_config = LoraConfig(
            r=args.rank, lora_alpha=args.rank, target_modules=["q_proj", "k_proj", "v_proj", "out_proj"]
        )
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            # there are only two options here. Either are just the prior attn processor layers
            # or there are the prior and text encoder atten layers
    ...
    if args.train_text_encoder:
        prior, text_encoder, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            prior, text_encoder, optimizer, train_dataloader, lr_scheduler
        )
    else:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(prior):
                ...
                model_input = image_encoder(pixel_values)
                model_input = model_input.add(1.0).div(42.0)

                # Sample noise that we'll add to the latents
                noise = torch.randn_like(model_input)
                bsz, channels, _, _ = model_input.shape
                # Sample a random timestep for each image
                timesteps = torch.rand((bsz,), device=model_input.device)
                ...
                model_pred = prior(noisy_model_input, timesteps, encoder_hidden_states)
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                ...
                scheduler_args = {}
                pipeline.scheduler = DDPMWuerstchenScheduler.from_config(pipeline.scheduler.config, **scheduler_args)
                ...
                if args.pre_compute_text_embeddings:
                    ...
                    pipeline_args = {"prompt": args.validation_prompt, "prior_timesteps": DEFAULT_STAGE_C_TIMESTEPS}
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            images = [
                pipeline(
                    args.validation_prompt, prior_timesteps=DEFAULT_STAGE_C_TIMESTEPS, generator=generator
                ).images[0]
                for _ in range(args.num_validation_images)
            ]
