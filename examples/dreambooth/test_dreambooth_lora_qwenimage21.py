...
class TestDreamBoothLoRAQwenImage21(ExamplesTestsAccelerate):
    instance_data_dir = "docs/source/en/imgs"
    instance_prompt = "photo"
    pretrained_model_name_or_path = "hf-internal-testing/tiny-qwenimage21-pipe"
    script_path = "examples/dreambooth/train_dreambooth_lora_qwenimage21.py"
    transformer_layer_type = "transformer_blocks.0.attn.to_k"

    def test_dreambooth_lora_qwenimage21(self):
    ...
    def test_dreambooth_lora_qwenimage21_checkpointing_checkpoints_total_limit(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            test_args = f"""
            {self.script_path}
            --pretrained_model_name_or_path={self.pretrained_model_name_or_path}
    ...
    def test_dreambooth_lora_qwenimage21_checkpointing_checkpoints_total_limit_removes_multiple_checkpoints(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            test_args = f"""
            {self.script_path}
            --pretrained_model_name_or_path={self.pretrained_model_name_or_path}
    ...
    def test_dreambooth_lora_qwenimage21_aspect_ratio_buckets(self):
        # Both latent dimensions have to be even for this model, so bucket sizes must divide by 32. That is a
        # model constraint rather than a preference, which is why this runs rather than being skipped.
        with tempfile.TemporaryDirectory() as tmpdir:
            test_args = f"""
                {self.script_path}
                --pretrained_model_name_or_path {self.pretrained_model_name_or_path}
    ...
    def test_dreambooth_lora_qwenimage21_caption_dropout(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            test_args = f"""
                {self.script_path}
                --pretrained_model_name_or_path {self.pretrained_model_name_or_path}
