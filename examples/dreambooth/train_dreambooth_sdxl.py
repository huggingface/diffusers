...
Text encoder was trained: {train_text_encoder}.
...
def main(args):
    ...
    # For mixed precision training we cast all non-trainable weights vae to half-precision
        ...
        if accelerator.is_main_process:
            for model in models:
                if isinstance(model, type(unwrap_model(unet))):
                    ...
                    # make sure to pop weight so that corresponding model is not saved agai
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("dreambooth-sd-xl", config=vars(args))
