...
def main(args):
    ...
    text_encoding_pipeline = Flux2KleinKVPipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        vae=None,
        transformer=None,
        tokenizer=tokenizer,
    ...
    def save_model_hook(models, weights, output_dir):
        ...
        if accelerator.is_main_process:
            ...
            Flux2KleinKVPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = Flux2KleinKVPipeline.lora_state_dict(input_dir)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                model_input = Flux2KleinKVPipeline._patchify_latents(model_input)
                model_input = (model_input - latents_bn_mean) / latents_bn_std

                cond_model_input = Flux2KleinKVPipeline._patchify_latents(cond_model_input)
                cond_model_input = (cond_model_input - latents_bn_mean) / latents_bn_std

                model_input_ids = Flux2KleinKVPipeline._prepare_latent_ids(model_input).to(device=model_input.device)
                cond_model_input_list = [cond_model_input[i].unsqueeze(0) for i in range(cond_model_input.shape[0])]
                cond_model_input_ids = Flux2KleinKVPipeline._prepare_image_ids(cond_model_input_list).to(
                ...
                packed_noisy_model_input = Flux2KleinKVPipeline._pack_latents(noisy_model_input)
                packed_cond_model_input = Flux2KleinKVPipeline._pack_latents(cond_model_input)
                ...
                model_pred = Flux2KleinKVPipeline._unpack_latents_with_ids(model_pred, model_input_ids)
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                # create pipeline
                pipeline = Flux2KleinKVPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    text_encoder=None,
                    tokenizer=None,
