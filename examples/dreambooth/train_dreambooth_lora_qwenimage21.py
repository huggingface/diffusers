...
# `vae_scale_factor * 2` for this VAE, checked against its config in `main`.
SIZE_MULTIPLE_OF = 32
...
class QwenImage21ValidationPipeline(QwenImage21Pipeline):
    """`QwenImage21Pipeline` that takes the image-pad mask alongside precomputed prompt embeddings.

    `__call__` gets that mask from `encode_prompt`, which it skips when handed `prompt_embeds`, so validation hands
    back the one it cached before the text encoder was freed.
    """
...
def save_model_card(
    ...
    model_description = f"""
# Qwen-Image 2.1 DreamBooth LoRA - {repo_id}
...
The weights were trained using [DreamBooth](https://dreambooth.github.io/) with the [Qwen-Image 2.1 diffusers trainer](https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/README_qwenimage21.md).
...
def log_validation(
        ...
        with autocast_ctx:
            image = pipeline(
                **pipeline_args,
                ...
                # Classifier-free guidance off, as in the model's own sample script: with `true_cfg_scale <= 1`
                # a step is a single forward pass through the transformer.
                true_cfg_scale=1.0,
    ...
    for tracker in accelerator.trackers:
        ...
        if tracker.name == "tensorboard":
            # The pipeline returns RGBA, this VAE having four channels, and `add_images` asserts three.
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=40,
        help="Denoising steps for validation images. 40 is what the model's own sample script uses.",
    )
    ...
    # An error rather than the pipeline's silent resize: a mismatch only shows up later, as a packing error.
    if args.resolution % SIZE_MULTIPLE_OF != 0:
        raise ValueError(f"--resolution must be a multiple of {SIZE_MULTIPLE_OF}, got {args.resolution}.")
    if args.aspect_ratio_buckets is not None:
        for height, width in parse_buckets_string(args.aspect_ratio_buckets):
            if height % SIZE_MULTIPLE_OF or width % SIZE_MULTIPLE_OF:
                raise ValueError(
                    f"every --aspect_ratio_buckets entry must be a multiple of {SIZE_MULTIPLE_OF}, got "
                    f"{height}x{width}."
                )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        # 32, not the usual 16: both latent dimensions have to be even to fill 2x2 slots.
        bucket_divisibility=SIZE_MULTIPLE_OF,
        ...
        for image in self.instance_images:
            ...
            # RGBA, not RGB: this VAE takes and returns four channels.
...
def main(args):
        ...
        if cur_class_images < args.num_class_images:
            ...
            for example in tqdm(
                ...
                for i, image in enumerate(images):
                    ...
                    # PNG, not JPEG: the pipeline returns RGBA and JPEG has no alpha channel.
    ...
    # A Qwen3-VL processor rather than a tokenizer: it also expands condition images into vision tokens.
    ...
    # Used as shipped: it sets `use_dynamic_shifting`, so the training sigmas stay unshifted.
    ...
    # 16 here, so one latent token covers a 16x16 pixel tile.
    vae_scale_factor = 2 ** len(vae.temperal_downsample)
    if vae_scale_factor * 2 != SIZE_MULTIPLE_OF:
        raise ValueError(
            f"This checkpoint's VAE scales by {vae_scale_factor}, so sizes must be multiples of "
            f"{vae_scale_factor * 2}, but the resolution checks used {SIZE_MULTIPLE_OF}."
        )
    ...
    text_encoding_pipeline = QwenImage21Pipeline.from_pretrained(
        args.pretrained_model_name_or_path,
        vae=None,
        transformer=None,
        processor=processor,
    ...
    def load_model_hook(models, input_dir):
        ...
        if not accelerator.distributed_type == DistributedType.DEEPSPEED:
            ...
            transformer_ = QwenImage21Transformer2DModel.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="transformer"
            )
            transformer_.add_adapter(transformer_lora_config)

        lora_state_dict = QwenImage21Pipeline.lora_state_dict(input_dir)
    ...
    def compute_text_embeddings(prompt, text_encoding_pipeline):
        with torch.no_grad():
            # The image-pad mask marks where condition-image tokens sit; the pipeline needs it alongside the embeddings.
            prompt_embeds, prompt_embeds_mask, image_pad_mask = text_encoding_pipeline.encode_prompt(prompt=prompt)
        return prompt_embeds, prompt_embeds_mask, image_pad_mask
    ...
    if args.caption_dropout > 0:
        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
            empty_prompt_embeds, empty_prompt_embeds_mask, _ = compute_text_embeddings("", text_encoding_pipeline)
    ...
    # The processor stays: it holds no weights, and the pipeline cannot be constructed without one.
    del text_encoder
    free_memory()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # One shape per image in the sequence; text to image has only the target.
                img_shapes = [[(1, latent_height, latent_width)]] * bsz
                # Latents are consumed unpatched, so packing is a plain spatial flatten.
                packed_noisy_model_input = QwenImage21Pipeline._pack_latents(
                    noisy_model_input,
                ...
                # `img_mask` marks which positions over [prompt tokens, target slots] stand for image latents, one
                # slot per 2x2 group of latents. The prompt half is all-False without condition images.
                target_slots = (latent_height * latent_width) // 4
                img_mask = torch.cat(
                    [
                        torch.zeros(bsz, prompt_embeds.shape[1], dtype=torch.bool, device=accelerator.device),
                        torch.ones(bsz, target_slots, dtype=torch.bool, device=accelerator.device),
                    ],
                    dim=1,
                )
                ...
                # The prediction spans the joint sequence, so keep the target's tail, as `__call__` does.
                model_pred = model_pred[:, -packed_noisy_model_input.shape[1] :]
                model_pred = QwenImage21Pipeline._unpack_latents(
        ...
        if accelerator.is_main_process:
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0:
                # create pipeline. The prompt is supplied as embeddings, so no text encoder is loaded.
                pipeline = QwenImage21ValidationPipeline.from_pretrained(
                    args.pretrained_model_name_or_path,
                    text_encoder=None,
                    processor=processor,
                ...
                pipeline.cached_image_pad_mask = validation_image_pad_mask
                images = log_validation(
                    pipeline=pipeline,
                    args=args,
                    accelerator=accelerator,
                    pipeline_args=validation_pipeline_args,
                    torch_dtype=weight_dtype,
                    epoch=epoch,
                )
                del pipeline
    ...
    if accelerator.is_main_process:
        ...
        if should_run_final_inference:
            ...
            # The transformer is reloaded, so this exercises the adapter that was written to disk.
            pipeline = QwenImage21ValidationPipeline.from_pretrained(
                args.pretrained_model_name_or_path,
                text_encoder=None,
                processor=processor,
