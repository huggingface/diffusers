...
def save_model_card(
    ...
    model_description = f"""
# Qwen-Image 2.1 image-to-image DreamBooth LoRA - {repo_id}
...
The weights were trained using [DreamBooth](https://dreambooth.github.io/) with the [Qwen-Image 2.1 image-to-image diffusers trainer](https://github.com/huggingface/diffusers/blob/main/examples/dreambooth/README_qwenimage21.md#image-to-image-editing).

## Trigger words

You should use `{instance_prompt}` as the edit instruction.
...
```py
    ...
    >>> condition = load_image("https://huggingface.co/datasets/huggingface/documentation-images/resolve/main/diffusers/cat.png")
    >>> image = pipe(f"{instance_prompt}", image=condition).images[0]
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Column in the dataset containing the condition image the edit is applied to. Required here.",
    )
    parser.add_argument(
        "--caption_column",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path or URL of the condition image to edit during validation.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="qwenimage21-img2img-dreambooth-lora",
    ...
    if args.dataset_name is None or args.cond_image_column is None:
        raise ValueError(
            "Image-to-image training pairs each target image with a condition image, so it needs a dataset that "
            "holds both: pass `--dataset_name` together with `--cond_image_column`. For text-to-image training use "
            "train_dreambooth_lora_qwenimage21.py."
        )

    if args.with_prior_preservation:
        raise ValueError(
            "`--with_prior_preservation` is not supported here. Class images carry a different prompt, and every "
            "sample in a batch has to share one image-pad layout, which a second prompt breaks."
        )

    if args.caption_dropout > 0:
        raise ValueError(
            "`--caption_dropout` is not supported here. Dropping a caption changes where the condition image's "
            "tokens land in the prompt, and the batch has to share one image-pad layout."
        )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if args.dataset_name is not None:
            ...
            if args.cond_image_column not in column_names:
                raise ValueError(
                    f"`--cond_image_column` value '{args.cond_image_column}' not found in dataset columns. Dataset "
                    f"columns are: {', '.join(column_names)}"
                )
            cond_images = dataset["train"][args.cond_image_column]
            if len(cond_images) != len(instance_images):
                raise ValueError(
                    f"The dataset has {len(instance_images)} target images but {len(cond_images)} condition images."
                )
        ...
        self.cond_images = []
        for img, cond_img in zip(instance_images, cond_images):
            self.instance_images.extend(itertools.repeat(img, repeats))
            self.cond_images.extend(itertools.repeat(cond_img, repeats))

        self.pixel_values = []
        self.cond_pixel_values = []
        self.cond_pil_images = []
        self.buckets = []
        bucket_to_idx = {}
        for image, cond_image in zip(self.instance_images, self.cond_images):
            image = exif_transpose(image)
            cond_image = exif_transpose(cond_image)
            # RGBA, not RGB: this VAE takes and returns four channels.
            if not image.mode == "RGBA":
                image = image.convert("RGBA")
            if not cond_image.mode == "RGBA":
                cond_image = cond_image.convert("RGBA")
            ...
            image, cond_tensor, cond_pil = self.train_transform_pair(
                image,
                cond_image,
                size=target,
            ...
            self.cond_pixel_values.append((cond_tensor, bucket_idx))
            # The vision-language model reads the condition image as pixels, so the cropped frame is kept as well.
            self.cond_pil_images.append(cond_pil)
    ...
    def __getitem__(self, index):
        ...
        example["cond_images"] = self.cond_pixel_values[index % self.num_instance_images][0]
        example["cond_pil_images"] = self.cond_pil_images[index % self.num_instance_images]
    ...
    def train_transform_pair(self, image, cond_image, size, center_crop=False, random_flip=False):
        """Resize and crop a target image and its condition image through one geometry, so an aligned pair stays
        aligned. Returns both tensors and the cropped condition image, which the prompt encoder needs."""
        target_height, target_width = size
        width, height = image.size
        scale = max(target_height / height, target_width / width)
        new_height, new_width = round(height * scale), round(width * scale)
        resize = lambda img: TF.resize(  # noqa: E731
            img, [new_height, new_width], interpolation=transforms.InterpolationMode.BILINEAR
        )
        image, cond_image = resize(image), resize(cond_image)
        if center_crop:
            image, cond_image = TF.center_crop(image, size), TF.center_crop(cond_image, size)
        else:
            i, j, h, w = transforms.RandomCrop.get_params(image, output_size=size)
            image, cond_image = TF.crop(image, i, j, h, w), TF.crop(cond_image, i, j, h, w)
        if random_flip and random.random() < 0.5:
            image, cond_image = TF.hflip(image), TF.hflip(cond_image)
        return (
            TF.normalize(TF.to_tensor(image), [0.5], [0.5]),
            TF.normalize(TF.to_tensor(cond_image), [0.5], [0.5]),
            cond_image,
        )
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    if cond_pixel_values.ndim == 4:
        cond_pixel_values = cond_pixel_values.unsqueeze(2)
    ...
    batch = {
        ...
        # Images rather than tensors: the prompt encoder feeds the condition image to the vision-language model.
        "cond_pil_images": [example["cond_pil_images"] for example in examples],
...
def main(args):
    ...
    # Prompt embeddings depend on the sample's condition image, so they are always precomputed per sample.
    ...
    def compute_text_embeddings(prompt, text_encoding_pipeline, cond_image=None):
        with torch.no_grad():
            # One sample at a time: a list of images applies to every prompt in the call, not pairwise.
            prompt_embeds, prompt_embeds_mask, image_pad_mask = text_encoding_pipeline.encode_prompt(
                prompt=prompt, image=None if cond_image is None else [cond_image]
            )
        return prompt_embeds, prompt_embeds_mask, image_pad_mask
    ...
    if effective_validation_prompt is not None:
        if args.validation_image is None:
            raise ValueError(
                "A validation prompt needs `--validation_image`, the image the edit is applied to. Pass it "
                "alongside `--validation_prompt` or `--final_validation_prompt`."
            )
        ...
        with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
            embeds, embeds_mask, image_pad_mask = compute_text_embeddings(
                effective_validation_prompt, text_encoding_pipeline, resized_validation_image
            )
        validation_pipeline_args = {
    ...
    if args.cache_latents:
        ...
        cond_latents_cache = [None] * train_dataset.num_instance_images
    ...
    image_pad_mask_cache = [None] * train_dataset.num_instance_images
            ...
            with torch.no_grad():
                ...
                if args.cache_latents:
                    with offload_models(vae, device=accelerator.device, offload=args.offload):
                        ...
                        instance_latents = vae.encode(batch["pixel_values"]).latent_dist.sample()
                        # Taken at the mode, not sampled: the condition image is not noised.
                        cond_latents = vae.encode(
                            batch["cond_pixel_values"].to(accelerator.device, non_blocking=True, dtype=vae.dtype)
                        ).latent_dist.mode()
                    for i, idx in enumerate(sample_indices):
                        instance_latents_cache[idx] = instance_latents[i : i + 1]
                        cond_latents_cache[idx] = cond_latents[i : i + 1]
                with offload_models(text_encoding_pipeline, device=accelerator.device, offload=args.offload):
                    for i, idx in enumerate(sample_indices):
                        prompt_embeds, prompt_embeds_mask, image_pad_mask = compute_text_embeddings(
                            batch["instance_prompts"][i], text_encoding_pipeline, batch["cond_pil_images"][i]
                        )
                        prompt_embeds_cache[idx] = prompt_embeds
                        prompt_embeds_mask_cache[idx] = prompt_embeds_mask
                        image_pad_mask_cache[idx] = image_pad_mask

        if args.cache_latents:
            assert all(latents is not None for latents in instance_latents_cache), "Latent cache has unfilled entries."
            assert all(latents is not None for latents in cond_latents_cache), (
                "Condition latent cache has unfilled entries."
            )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(models_to_accumulate):
                # Each sample's embeddings were encoded with its own condition image, gathered by dataset index.
                prompt_pairs = [(prompt_embeds_cache[idx], prompt_embeds_mask_cache[idx]) for idx in sample_indices]
                prompt_embeds, prompt_embeds_mask = concat_prompt_embedding_batches(*prompt_pairs)

                # The transformer reads one image-pad layout for the whole batch, so the samples have to agree.
                batch_image_pad_masks = [image_pad_mask_cache[idx] for idx in sample_indices]
                if any(not torch.equal(mask, batch_image_pad_masks[0]) for mask in batch_image_pad_masks[1:]):
                    raise ValueError(
                        "The samples in this batch place the condition image's tokens differently, which happens "
                        "when their prompts differ in length. Train with `--train_batch_size 1`, or give the "
                        "samples in a batch the same prompt."
                    )
                image_pad_mask = batch_image_pad_masks[0].repeat(len(sample_indices), 1).to(accelerator.device)

                # Convert images to latent space
                if args.cache_latents:
                    model_input = torch.cat([instance_latents_cache[idx] for idx in sample_indices], dim=0)
                    cond_model_input = torch.cat([cond_latents_cache[idx] for idx in sample_indices], dim=0)
                else:
                    with offload_models(vae, device=accelerator.device, offload=args.offload):
                ...
                # Clean, at the same normalisation as the target.
                cond_model_input = ((cond_model_input - latents_mean) * latents_std).to(dtype=weight_dtype)
                ...
                cond_latent_height, cond_latent_width = cond_model_input.shape[3], cond_model_input.shape[4]
                # One shape per image in the sequence, ordered [condition, target].
                img_shapes = [[(1, cond_latent_height, cond_latent_width), (1, latent_height, latent_width)]] * bsz
                ...
                packed_cond_model_input = QwenImage21Pipeline._pack_latents(
                    cond_model_input,
                    batch_size=cond_model_input.shape[0],
                    num_channels_latents=cond_model_input.shape[1],
                    height=cond_latent_height,
                    width=cond_latent_width,
                )
                # Condition latents lead, noisy target follows, the order `img_shapes` declares.
                packed_input = torch.cat([packed_cond_model_input, packed_noisy_model_input], dim=1)
                # `img_mask` marks which positions over [prompt tokens, target slots] stand for image latents, one
                # slot per 2x2 group. The prompt half comes from the encoder, where the vision tokens sit.
                target_slots = (latent_height * latent_width) // 4
                cond_slots = (cond_latent_height * cond_latent_width) // 4
                if int(image_pad_mask[0].sum()) != cond_slots:
                    raise ValueError(
                        f"The prompt carries {int(image_pad_mask[0].sum())} condition-image slots but the condition "
                        f"latents need {cond_slots}. The vision-language processor resizes images below its minimum "
                        f"pixel count, so a condition image this small ({cond_latent_height * 16}x"
                        f"{cond_latent_width * 16}) does not line up. Train at a larger resolution."
                    )
                img_mask = torch.cat(
                    [
                        image_pad_mask,
                        torch.ones(bsz, target_slots, dtype=image_pad_mask.dtype, device=accelerator.device),
                    ],
                    dim=1,
                )
