...
from diffusers import DDIMInverseScheduler, DDIMScheduler, StableDiffusionPipeline, StableDiffusionPix2PixZeroPipeline
...
class SVDiffModule(nn.Module):
    ...
    def _reshape_weight_for_svd(
        self, updated_weight: torch.Tensor, original_weight: torch.Tensor = None, reverse: bool = False, **kwargs
    ):
            ...
            for key in f.keys():
                ...
                if accepts_dtype:
                    set_module_tensor_to_device(
                        svdiff_modules[key], "delta", param_device, value=f.get_tensor(key), dtype=torch_dtype
                    )
                else:
