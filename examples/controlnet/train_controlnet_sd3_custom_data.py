...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--dataset_preprocess_batch_size", type=int, default=1500, help="Batch size for preprocessing dataset."
    )
    parser.add_argument(
        "--validation_prompt",
        type=str,
...
class CustomDataset(Dataset):
    def __init__(self, dataset, args, image_transforms, conditioning_image_transforms, tokenizers, text_encoders, accelerator):
    ...
    def __getitem__(self, idx):
        ...
        conditioning_image = example[self.conditioning_image_column]
        ...
        # Process captions
        captions = self.process_captions(example[self.caption_column])
    ...
    def process_captions(self, captions):
        if random.random() < self.proportion_empty_prompts:
            ...
            return captions
        elif isinstance(captions, (list, np.ndarray)):
            return random.choice(captions)  # Choose a random caption
        else:
            raise ValueError(f"Caption column should contain strings or lists of strings.")
    
# Main function to create the dataset
def make_train_dataset(args, tokenizer_list, text_encoder_list, accelerator):
    dataset = None
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset("json", data_files=args.jsonl_for_train, cache_dir=args.cache_dir)["train"]
        dataset = dataset.flatten_indices()
    else:
        raise ValueError("Either `args.dataset_name` or `args.jsonl_for_train` must be provided.")

    # Set column names
    column_names = dataset.column_names
    args.image_column = args.image_column or column_names[0]
    args.caption_column = args.caption_column or column_names[1]
    args.conditioning_image_column = args.conditioning_image_column or column_names[2]
    ...
    # Apply sampling if max_train_samples is specified
    with accelerator.main_process_first():
        if args.max_train_samples is not None:
            dataset = dataset.shuffle(seed=args.seed).select(range(args.max_train_samples))

    # Wrap the dataset with CustomDataset
    train_dataset = CustomDataset(
        ...
        text_encoders=text_encoder_list, 
...
def main(args):
    ...
    with accelerator.main_process_first():
        train_dataset = make_train_dataset(args, tokenizers, text_encoders, accelerator)
    ...
    lr_scheduler = get_scheduler(
        args.lr_scheduler,
        optimizer=optimizer,
        num_warmup_steps=int(args.lr_warmup_steps / accelerator.num_processes),
        num_training_steps=int(args.max_train_steps / accelerator.num_processes),
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                prompt_encode = compute_text_embeddings(batch, text_encoders, tokenizers)
                prompt_embeds = prompt_encode["prompt_embeds"]
                pooled_prompt_embeds = prompt_encode["pooled_prompt_embeds"]
