...
from diffusers.training_utils import cast_training_params, free_memory, compute_density_for_timestep_sampling, compute_loss_weighting_for_sd3
...
def log_validation(
    ...
    pipe.scheduler = FlowMatchEulerDiscreteScheduler.from_config(pipe.scheduler.config, **scheduler_args)
...
def main(args):
    ...
    tokenizer = LlamaTokenizerFast.from_pretrained(args.pretrained_model_name_or_path, subfolder="tokenizer")
    text_encoder = LlamaModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder")
    tokenizer_2 = CLIPTokenizer.from_pretrained(args.pretrained_model_name_or_path, subfolder="tokenizer_2")
    text_encoder_2 = CLIPTextModel.from_pretrained(args.pretrained_model_name_or_path, subfolder="text_encoder_2")
    ...
    scheduler_sigmas = scheduler.sigmas.clone().to(device=accelerator.device, dtype=weight_dtype)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                indices = (weights * scheduler.config.num_train_timesteps).long()
                ...
                timesteps = (scheduler_sigmas[indices].flatten() * 1000.0).long()
                guidance = torch.tensor([args.guidance_scale] * model_input.shape[0], dtype=weight_dtype, device=accelerator.device) * 1000.0

                # Predict the noise residual
                weights = compute_loss_weighting_for_sd3(weighting_scheme=args.flow_weighting_scheme, sigmas=sigmas)
