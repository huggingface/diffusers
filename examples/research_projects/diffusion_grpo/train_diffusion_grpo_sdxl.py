...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="GRPO KL Divergence penalty.",
    )
    parser.add_argument(
        "--num_generations",
        type=int,
        default=2,
        help="Number of generations per prompt (G in the GRPO paper). Must evenly divide the global batch size.",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
    ...
    parser.add_argument(
        "--tracker_name",
        type=str,
        default="diffusion-grpo-lora-sdxl",
...
def compute_grpo_loss(model_pred, target, model_losses, ref_losses, advantages, beta_grpo):
    """
    Compute GRPO loss based on model predictions, reference predictions, and advantages
    """
    # Compute KL divergence between model and reference predictions
    per_token_kl = torch.exp(ref_losses - model_losses) - (ref_losses - model_losses) - 1
    
    # Compute policy gradient term with advantages
    policy_term = torch.exp(model_losses - model_losses.detach()) * advantages.unsqueeze(1)
    
    # Combine policy gradient and KL terms
    loss = -(policy_term - beta_grpo * per_token_kl)
