...
def convert_ldm_unet_checkpoint(
    ...
    if skip_extract_state_dict:
        ...
        if sum(k.startswith("model_ema") for k in keys) > 100 and extract_ema:
            ...
            for key in keys:
                if (
                    key.startswith(unet_key)
                    and not key.startswith("control_model.glyph_block")
                    and not key.startswith("control_model.position_block")
                    and not key.startswith("control_model.fuse_block")
                ):
