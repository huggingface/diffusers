...
class CLIPGuidedImagesMixingStableDiffusion(DiffusionPipeline):
    """
    Based on:
    https://github.com/huggingface/diffusers/blob/main/examples/community/clip_guided_stable_diffusion_img2img.py
    """
    def __init__(
        self,
        vae: AutoencoderKL,
    ...
    def get_image_description(self, image):
        ...
        return self.coca_tokenizer.decode(generated[0].cpu().numpy()).split("<end_of_text>")[0].replace("<start_of_text>", "").rstrip(" .,")
    ...
    def __call__(
        ...
        coca_is_none = [x[0] for x in coca_is_none if x[1] == True]
        coca_is_none_str = ', '.join(coca_is_none)
        # generate prompts with coca model
        if content_prompt is None:
            if len(coca_is_none):
                raise ValueError(f"Content prompt is None and CoCa [{coca_is_none_str}] is None. Set prompt or pass Coca [{coca_is_none_str}] to DiffusionPipeline.")
            content_prompt = self.get_image_description(content_image)
        if style_prompt is None:
            if len(coca_is_none):
                raise ValueError(f"Style prompt is None and CoCa [{coca_is_none_str}] is None. Set prompt or pass Coca [{coca_is_none_str}] to DiffusionPipeline.")
        ...
        if clip_guidance_scale > 0:
            ...
            clip_image_embeddings = slerp(slerp_clip_image_style_strength, content_clip_image_embedding, style_clip_image_embedding)
