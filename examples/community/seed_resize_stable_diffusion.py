...
class SeedResizeStableDiffusionPipeline(DiffusionPipeline, StableDiffusionMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        w = (
            latents_shape_reference[3]
            if dx >= 0
            else latents_shape_reference[3] + 2 * dx
        )
        h = (
            latents_shape_reference[2]
            if dy >= 0
            else latents_shape_reference[2] + 2 * dy
        )
        ...
        # import pdb
        # pdb.set_trace()
        latents[:, :, ty : ty + h, tx : tx + w] = latents_reference[
            :, :, dy : dy + h, dx : dx + w
        ]
