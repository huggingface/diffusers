...
def translate_prompt(prompt, translation_tokenizer, translation_model, device):
    ...
    en_trans = translation_tokenizer.batch_decode(
        ...
        **kwargs,
        ...
        if batch_size == 1 and prompt_language != "en":
            prompt = translate_prompt(
                prompt, self.translation_tokenizer, self.translation_model, self.device
            )
        ...
        if do_classifier_free_guidance:
            ...
            if negative_prompt is None:
                ...
                negative_prompt_language = detect_language(
                    self.detection_pipeline, negative_prompt, batch_size
                )
                ...
                if isinstance(negative_prompt, list):
                    negative_prompt_languages = detect_language(
                        self.detection_pipeline, negative_prompt, batch_size
                    )
                    for index in range(batch_size):
