    ...
    def __call__(
        ...
        def generate(
            ...
            for i, t in enumerate(self.progress_bar(decoder_timesteps_tensor)):
                # expand the latents if we are doing classifier free guidance
                latent_model_input = (
                    torch.cat([decoder_latents] * 2) if do_classifier_free_guidance else decoder_latents
                )
        ...
        batch_attention_mask = torch.max(inputs.attention_mask[0], inputs.attention_mask[1])
        batch_attention_mask = torch.cat([batch_attention_mask.unsqueeze(0)] * steps).to(device)
