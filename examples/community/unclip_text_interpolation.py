...
class UnCLIPTextInterpolationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        attention_mask = torch.max(inputs.attention_mask[0], inputs.attention_mask[1])
        attention_mask = attention_mask.unsqueeze(0).to(device)

        start_text_embeds = text_model_output.text_embeds[0]
        end_text_embeds = text_model_output.text_embeds[1]

        start_last_hidden_state = text_model_output.last_hidden_state[0]
        end_last_hidden_state = text_model_output.last_hidden_state[1]
        ...
        for interp_val in np.linspace(0, 1, steps):
            ...
            if isinstance(res, ImagePipelineOutput):
                out.append(res.images[0])
            elif isinstance(res, tuple):
                out.append(res[0][0])

        return ImagePipelineOutput(images=out) if return_dict else (out,)
