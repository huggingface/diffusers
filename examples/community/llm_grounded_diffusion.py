...
class AttnProcessorWithHook(AttnProcessor2_0):
    ...
    def __call__(
        ...
        if self.fast_attn:
            ...
            key = key.view(batch_size, -1, attn.heads,
                           head_dim).transpose(1, 2)
...
class LLMGroundedDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, IPAdapterMixin, FromSingleFileMixin):
    r"""
    ...
    def check_inputs(
        ...
        if prompt is not None and prompt_embeds is not None:
            ...
            raise ValueError(
                "If the prompt is None, the phrase_indices cannot be None")
    ...
    def add_ca_loss_per_attn_map_to_loss(
        ...
        for obj_idx in range(object_number):
            ...
            for obj_box in obj_boxes:
                ...
                x_min, y_min, x_max, y_max = scale_proportion(
                    obj_box, H=H, W=W)
    ...
    def __call__(
        ...
        cond_boxes = cond_boxes.unsqueeze(
        ...
        num_grounding_steps = int(
            gligen_scheduled_sampling_beta * len(timesteps))
            ...
            try:
                ...
                while (
                    ...
                    if guidance_callback is not None:
                        guidance_callback(
                            self, latents, loss, iteration, index)
