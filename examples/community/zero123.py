...
class Zero123Pipeline(DiffusionPipeline):
    ...
    def _encode_image(self, image, elevation, azimuth, distance, device, num_images_per_prompt, do_classifier_free_guidance):
        ...
        camera_embeddings = torch.as_tensor([
            math.radians(elevation), math.sin(math.radians(azimuth)), math.cos(math.radians(azimuth)), distance
        ]).to(image_embeddings)[None,None,:].repeat(bs_embed, 1, 1)
    ...
    def _get_latent_model_input(
        ...
        if isinstance(image, PIL.Image.Image):
            ...
            image_pt = torch.stack([TF.to_tensor(img) for img in image], dim=0).to(latents)
        else:
            image_pt = image
        
        image_pt = image_pt * 2. - 1. # scale to [-1, 1]
    ...
    def __call__(
        ...
        image_embeddings = self._encode_image(pil_image, elevation, azimuth, distance, device, num_images_per_prompt, do_classifier_free_guidance)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = self._get_latent_model_input(latents, image, num_images_per_prompt, do_classifier_free_guidance)
