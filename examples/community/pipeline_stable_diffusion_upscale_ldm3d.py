...
class StableDiffusionUpscaleLDM3DPipeline(
    ...
    def __init__(
        ...
        self.image_processor = VaeImageProcessorLDM3D(
            vae_scale_factor=self.vae_scale_factor, resample="bilinear"
        )
    ...
    def run_safety_checker(self, image, device, dtype):
        if self.safety_checker is None:
            ...
            safety_checker_input = self.feature_extractor(
                rgb_feature_extractor_input, return_tensors="pt"
            ).to(device)
    ...
    def __call__(
        ...
        latent_space_image = torch.cat(
            [latent_space_image] * batch_multiplier * num_images_per_prompt
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat(
                    [latent_model_input, latent_space_image], dim=1
                )
        ...
        if not output_type == "latent":
            ...
            image = self.vae.decode(
                latents / self.vae.scaling_factor, return_dict=False
            )[0]
