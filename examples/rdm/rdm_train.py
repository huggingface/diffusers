...
from transformers import CLIPFeatureExtractor, CLIPTextModel, CLIPTokenizer, CLIPModel, CLIPFeatureExtractor, CLIPTokenizer
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the dataset image column.",
    )
    parser.add_argument(
        "--dataset_txt_col",
        type=str,
        required=True,
        help="Name of the dataset text column.",
    )
    parser.add_argument(
        "--dataset_save_path",
        type=str,
...
class RDMDataset(Dataset):
    ...
    def __getitem__(self, i):
        example = {}
        image = self.dataset[self.img_col][i]

        if not image.mode == "RGB":
            image = image.convert("RGB")
        text = self.dataset[self.txt_col][i]
        ...
        example["nearest_neighbors"] = self.retriever.get_knn_from_text(text).examples[self.img_col]
...
def main():
    ...
    retriever = Retriever(clip_model, tokenizer, feature_extractor, dataset, args.dataset_save_path, accelerator.device, args.dataset_img_col)
    train_dataset = RDMDataset(dataset,args.dataset_img_col,args.dataset_txt_col,tokenizer,retriever,size=512)
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("rdm_training", config=vars(args))
