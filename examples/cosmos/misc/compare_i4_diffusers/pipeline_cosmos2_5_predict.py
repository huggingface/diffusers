    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                print('Time:', i)
                print('Timestep Difference:', (timesteps_copy-in_timestep).abs().amax().item())
                diff = (in_latents_copy-in_latents).abs()
                print('In Latents Difference:', diff.amax().item(), diff.mean().item())
                diff = (cosmos_out - noise_pred).abs()
                print('Out Difference:', diff.amax().item(), diff.mean().item())
                print('-'*100)
