...
name = "lemons"
if not os.path.exists(f"{name}_ldm3d_rgb.jpg"):
    pipe_ldm3d = StableDiffusionLDM3DPipeline.from_pretrained("Intel/ldm3d-4c")
    pipe_ldm3d.to("cuda")

    prompt =f"A picture of some {name} on a table"
...
pipe_ldm3d_upscale = StableDiffusionUpscaleLDM3DPipeline.from_pretrained("/export/share/projects/mcai/ldm3d/hf_ckpt/ldm3d-hr-fs")
...
outputs = pipe_ldm3d_upscale(prompt="", rgb=low_res_img, depth=low_res_depth, num_inference_steps=50)
