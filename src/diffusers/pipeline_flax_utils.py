...
class FlaxDiffusionPipeline(ConfigMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], **kwargs):
        ...
        <Tip>
         ...
         models](https://huggingface.co/docs/hub/models-gated#gated-models), *e.g.* `"runwayml/stable-diffusion-v-1-5"`
        ...
        >>> pipeline = FlaxDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v-1-5")

        >>> # Download pipeline, but overwrite scheduler
        >>> from diffusers import LMSDiscreteScheduler

        >>> scheduler = LMSDiscreteScheduler(beta_start=0.00085, beta_end=0.012, beta_schedule="scaled_linear")
        >>> pipeline = FlaxDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v-1-5", scheduler=scheduler)
        ```
        """
