...
def compute_snr(noise_scheduler, timesteps):
    ...
    alphas_cumprod = alphas_cumprod.to(device=timesteps.device)[timesteps].float()
    while len(alphas_cumprod.shape) < len(timesteps.shape):
        alphas_cumprod = alphas_cumprod[..., None]
    alpha_squared = alphas_cumprod.expand(timesteps.shape)

    sigma_squared = 1 - alpha_squared

    # Compute SNR.
    snr = alpha_squared / sigma_squared
    return snr
