...
# auto_docstring
class QwenImageNAGTextInputsStep(ModularPipelineBlocks):
    """
    Optional text input processing step for Normalized Attention Guidance (NAG).

    This step batch-expands `nag_negative_prompt_embeds` (and its mask) to match the final batch size
    (`batch_size * num_images_per_prompt`).

    This block should be placed after the text encoder step (so the embeddings are available) and before denoising.

    Inputs:
        num_images_per_prompt (`int`, *optional*, defaults to 1):
            The number of images to generate per prompt.
        batch_size (`int`):
            Base batch size (number of prompts). Can be generated by `QwenImageTextInputsStep`.
        nag_negative_prompt_embeds (`Tensor`, *optional*):
            NAG negative prompt embeddings.
        nag_negative_prompt_embeds_mask (`Tensor`, *optional*):
            Mask for NAG negative prompt embeddings.

    Outputs:
        nag_negative_prompt_embeds (`Tensor`):
            The NAG negative prompt embeddings. (batch-expanded)
        nag_negative_prompt_embeds_mask (`Tensor`):
            The NAG negative prompt embeddings mask. (batch-expanded)
    """
    ...
    def description(self) -> str:
        return (
            "Optional text input processing step for Normalized Attention Guidance (NAG).\n"
            "This step:\n"
            "  1. Expands NAG embeddings to match final batch size"
        )
    ...
    def inputs(self) -> List[InputParam]:
        return [
            ...
            InputParam.template("nag_negative_prompt_embeds"),
            InputParam.template("nag_negative_prompt_embeds_mask"),
        ]

    @property
    def intermediate_outputs(self) -> List[OutputParam]:
        return [
            OutputParam.template("nag_negative_prompt_embeds", note="batch-expanded"),
            OutputParam.template("nag_negative_prompt_embeds_mask", note="batch-expanded"),
        ]
    ...
    def __call__(self, components: QwenImageModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if nag_negative_prompt_embeds_mask is None:
            nag_negative_prompt_embeds_mask = torch.ones(
                nag_negative_prompt_embeds.shape[:2],
                dtype=torch.bool,
                device=nag_negative_prompt_embeds.device,
            )

        block_state.nag_negative_prompt_embeds = repeat_tensor_to_batch_size(
            "nag_negative_prompt_embeds",
            nag_negative_prompt_embeds,
            batch_size=block_state.batch_size,
            num_images_per_prompt=block_state.num_images_per_prompt,
        )
        block_state.nag_negative_prompt_embeds_mask = repeat_tensor_to_batch_size(
