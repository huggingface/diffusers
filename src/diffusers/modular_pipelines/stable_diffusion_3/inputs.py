...
class SD3TextInputStep(ModularPipelineBlocks):
    ...
    def __call__(self, components: SD3ModularPipeline, state: PipelineState) -> PipelineState:
        ...
        if block_state.do_classifier_free_guidance and block_state.negative_prompt_embeds is not None:
            ...
            negative_prompt_embeds = negative_prompt_embeds.view(block_state.batch_size * block_state.num_images_per_prompt, neg_seq_len, -1)
...
class SD3AdditionalInputsStep(ModularPipelineBlocks):
    ...
    def __init__(self, image_latent_inputs: list[str] = ["image_latents"], additional_batch_inputs: list[str] =[]):
        self._image_latent_inputs = image_latent_inputs if isinstance(image_latent_inputs, list) else [image_latent_inputs]
        self._additional_batch_inputs = additional_batch_inputs if isinstance(additional_batch_inputs, list) else[additional_batch_inputs]
        super().__init__()
