...
class AnimaImg2ImgCoreDenoiseStep(SequentialPipelineBlocks):
    ...
    block_names = ["text_conditioning", "input", "prepare_latents", "denoise"]
...
class AnimaAutoDenoiseStep(AutoPipelineBlocks):
    ...
    ``image_latents`` are present. - `AnimaImg2ImgDenoiseStep` (img2img) is used when ``image_latents``
    are present (set upstream by ``AnimaAutoVaeImageEncoderStep``).
    ...
    def description(self) -> str:
        return (
            ...
            " - `AnimaImg2ImgDenoiseStep` (img2img) is used when image_latents are present."
        )
