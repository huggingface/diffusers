...
def _convert_stabilityai_control_lora_to_diffusers(checkpoint):
    ...
    for k, v in controlnet_state_dict.items():
        if ".down" in k:
            new_new_checkpoint[k.replace(".down", "lora_A.weight")] = v
        elif ".up" in k:
            new_new_checkpoint[k.replace(".up", "lora_B.weight")] = v
        new_new_checkpoint[k] = v

    return new_new_checkpoint
