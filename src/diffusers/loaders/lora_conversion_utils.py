...
def _convert_bfl_flux_control_lora_to_diffusers(old_state_dict):
    ...
    def remap_double_blocks(key, converted_state_dict, original_state_dict):
        block_index = int(key.split(".")[1])

        if "img_attn.qkv.lora_A" in key:
            sample_lora_weight = original_state_dict.pop(f"double_blocks.{block_index}.img_attn.qkv.lora_A.weight")
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_v.lora_A.weight"] = torch.cat(
                [sample_lora_weight]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_q.lora_A.weight"] = torch.cat(
                [sample_lora_weight]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_k.lora_A.weight"] = torch.cat(
                [sample_lora_weight]
            )

        elif "txt_attn.qkv.lora_A" in key:
            context_lora_weight = original_state_dict.pop(f"double_blocks.{block_index}.txt_attn.qkv.lora_A.weight")
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_q_proj.lora_A.weight"] = torch.cat(
                [context_lora_weight]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_k_proj.lora_A.weight"] = torch.cat(
                [context_lora_weight]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_v_proj.lora_A.weight"] = torch.cat(
                [context_lora_weight]
            )

        elif "img_attn.qkv.lora_B.weight" in key:
            sample_q, sample_k, sample_v = torch.chunk(
                original_state_dict.pop(f"double_blocks.{block_index}.img_attn.qkv.lora_B.weight"), 3, dim=0
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_q.lora_B.weight"] = torch.cat([sample_q])
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_k.lora_B.weight"] = torch.cat([sample_k])
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_v.lora_B.weight"] = torch.cat([sample_v])

        elif "img_attn.qkv.lora_B.bias" in key:
            sample_q_bias, sample_k_bias, sample_v_bias = torch.chunk(
                original_state_dict.pop(f"double_blocks.{block_index}.img_attn.qkv.lora_B.bias"), 3, dim=0
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_q.lora_B.bias"] = torch.cat(
                [sample_q_bias]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_k.lora_B.bias"] = torch.cat(
                [sample_k_bias]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.to_v.lora_B.bias"] = torch.cat(
                [sample_v_bias]
            )

        elif "txt_attn.qkv.lora_B.weight" in key:
            context_q, context_k, context_v = torch.chunk(
                original_state_dict.pop(f"double_blocks.{block_index}.txt_attn.qkv.lora_B.weight"), 3, dim=0
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_q_proj.lora_B.weight"] = torch.cat(
                [context_q]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_k_proj.lora_B.weight"] = torch.cat(
                [context_k]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_v_proj.lora_B.weight"] = torch.cat(
                [context_v]
            )

        elif "txt_attn.qkv.lora_B.bias" in key:
            context_q_bias, context_k_bias, context_v_bias = torch.chunk(
                original_state_dict.pop(f"double_blocks.{block_index}.txt_attn.qkv.lora_B.bias"), 3, dim=0
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_q_proj.lora_B.bias"] = torch.cat(
                [context_q_bias]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_k_proj.lora_B.bias"] = torch.cat(
                [context_k_bias]
            )
            converted_state_dict[f"transformer_blocks.{block_index}.attn.add_v_proj.lora_B.bias"] = torch.cat(
                [context_v_bias]
            )
    ...
    def remap_single_blocks(key, converted_state_dict, original_state_dict):
        ...
        # Hardcoded for now. Can try to infer from state dict in future
        inner_dim = 3072
        mlp_ratio = 4.0
        mlp_hidden_dim = int(inner_dim * mlp_ratio)
        split_size = (inner_dim, inner_dim, inner_dim, mlp_hidden_dim)

        if "linear1.lora_A.weight" in key:
            lora_weight = original_state_dict.pop(f"single_blocks.{block_index}.linear1.lora_A.weight")
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_q.lora_A.weight"] = torch.cat(
                [lora_weight]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_k.lora_A.weight"] = torch.cat(
                [lora_weight]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_v.lora_A.weight"] = torch.cat(
                [lora_weight]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.proj_mlp.lora_A.weight"] = torch.cat(
                [lora_weight]
            )

        elif "linear1.lora_B.weight" in key:
            q, k, v, mlp = torch.split(
                original_state_dict.pop(f"single_blocks.{block_index}.linear1.lora_B.weight"), split_size, dim=0
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_q.lora_B.weight"] = torch.cat([q])
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_k.lora_B.weight"] = torch.cat([k])
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_v.lora_B.weight"] = torch.cat([v])
            converted_state_dict[f"single_transformer_blocks.{block_index}.proj_mlp.lora_B.weight"] = torch.cat([mlp])

        elif "linear1.lora_B.bias" in key:
            q_bias, k_bias, v_bias, mlp_bias = torch.split(
                original_state_dict.pop(f"single_blocks.{block_index}.linear1.lora_B.bias"), split_size, dim=0
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_q.lora_B.bias"] = torch.cat(
                [q_bias]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_k.lora_B.bias"] = torch.cat(
                [k_bias]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.attn.to_v.lora_B.bias"] = torch.cat(
                [v_bias]
            )
            converted_state_dict[f"single_transformer_blocks.{block_index}.proj_mlp.lora_B.bias"] = torch.cat(
                [mlp_bias]
            )
    ...
    def remap_final_layer(key, converted_state_dict, original_state_dict):
        new_key = key.replace("final_layer.linear", "proj_out")
        new_key = new_key.replace("final_layer.adaLN_modulation.1", "norm_out.linear")
        converted_state_dict[new_key] = swap_scale_shift(original_state_dict.pop(key))
    ...
    TRANSFORMER_SPECIAL_KEYS_REMAP = {
        "double_blocks": remap_double_blocks,
        "single_blocks": remap_single_blocks,
        "final_layer": remap_final_layer,
    }
    ...
    for key in list(old_state_dict.keys()):
        ...
        if new_key != key:
            new_state_dict[new_key] = old_state_dict.pop(key)

    for key in list(old_state_dict.keys()):
        for special_key, handler_fn_inplace in TRANSFORMER_SPECIAL_KEYS_REMAP.items():
            if special_key not in key:
                continue
            handler_fn_inplace(key, new_state_dict, old_state_dict)

    # prefix everything with transformer
    for key in list(new_state_dict.keys()):
        new_state_dict[f"transformer.{key}"] = new_state_dict.pop(key)

    if len(old_state_dict) > 0:
        raise ValueError(f"`original_state_dict` should be empty at this point but has {old_state_dict.keys()=}.")
