...
def flux2_conversion(config):
    ...
    for name in ("double_stream_modulation_img", "double_stream_modulation_txt", "single_stream_modulation"):
        modules.append((name + ".lin", name + ".linear"))
    for i in range(config["num_layers"]):
        ...
        for modality, attention, mlp, norms in (
            ...
            modules.extend(
                [
                    (f"{old}.{modality}_mlp.0", f"{new}.{mlp}." + "linear_in"),
                    (f"{old}.{modality}_mlp.2", f"{new}.{mlp}." + "linear_out"),
                    (
    ...
    for i in range(config["num_single_layers"]):
        ...
        modules.extend([(old + ".linear1", new + ".attn.to_qkv_mlp_proj"), (old + ".linear2", new + ".attn.to_out")])
