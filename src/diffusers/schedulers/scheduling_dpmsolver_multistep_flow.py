...
class DPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        if self.config.use_karras_sigmas:
            ...
            sigmas = self._convert_to_flow(in_sigmas=sigmas, num_inference_steps=self.num_inference_steps, shift=self.config.timestep_shift)
            timesteps = sigmas
        else:
    ...
    def _convert_to_flow(self, in_sigmas: torch.Tensor, num_inference_steps, shift: float = 1.0) -> torch.Tensor:
        if hasattr(self.config, "sigma_min"):
            sigma_min = self.config.sigma_min
        else:
            sigma_min = None
        ...
        betas = torch.linspace(sigma_min, sigma_max, N + 1).to(in_sigmas.device)
        sigmas = 1.0 - betas
