...
@dataclass
class DPMEncoderSchedulerOutput(BaseOutput):
    """
...
class DPMEncoderScheduler(SchedulerMixin, ConfigMixin):
    """
    DPM-Encoder is a reverse scheduler. It should be treated as the forward version of DDIMScheduler.
    [`~ConfigMixin`] takes care of storing all config attributes that are passed in the scheduler's `__init__`
    function, such as `num_train_timesteps`. They can be accessed via `scheduler.config.num_train_timesteps`.
    [`~ConfigMixin`] also provides general loading and saving functionality via the [`~ConfigMixin.save_config`] and
    [`~ConfigMixin.from_config`] functions.
    For more details, see the paper where it is proposed: https://arxiv.org/abs/2210.05559
    Args:
    ...
    def step(
    ...
    ) -> Union[DPMEncoderSchedulerOutput, Tuple]:
        """
        Predict the sample at the next timestep by using the encoding SDE.
        Args:
            ...
            return_dict (`bool`): option for returning tuple rather than DPMEncoderSchedulerOutput class
        Returns:
            [`~schedulers.scheduling_dpm_encoder.DPMEncoderSchedulerOutput`] or `tuple`:
            [`~schedulers.scheduling_dpm_encoder.DPMEncoderSchedulerOutput`] if `return_dict` is True, otherwise a `tuple`. When
        ...
        return DPMEncoderSchedulerOutput(
