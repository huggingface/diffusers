...
class HeunDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def scale_model_input(
        ...
        """
        Scales the denoising model input by `(sigma**2 + 1) ** 0.5` to match the K-Heun algorithm.
    ...
    def step(
        ...
        Args:
            ...
            return_dict (`bool`): option for returning tuple rather than HeunDiscreteSchedulerOutput class

        Returns:
            [`~schedulers.scheduling_utils.HeunDiscreteSchedulerOutput`] or `tuple`:
            [`~schedulers.scheduling_utils.HeunDiscreteSchedulerOutput`] if `return_dict` is True, otherwise a `tuple`.
