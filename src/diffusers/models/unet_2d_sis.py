...
def get_config(img_size:int=512,in_channels:int=3,out_channels:int=3,cls_count:int=None):
    ...
    SIS_CONFIG_512 = dict(
        ...
        cls_count=cls_count,
        img_size=512,
        down_block_types = ("ConvBlock", "DownBlock", "DownBlock", "DownBlock", "DownAttnBlock", "DownAttnBlock", "DownAttnBlock"),
    ...
    assert str(img_size) in configuration.keys(),"img_size should be in existing configurations {configuration.keys()}"
...
class UNet2DSISModel(ModelMixin,ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        assert len(down_block_types)==len(up_block_types),"Model should be symetric and have the same number of up and down blocks"
        assert len(block_out_channels) == len(down_block_types),"Block channels and block names should have the same number of items..."
        ...
        for i,blk_out_channels in enumerate(encoder_channels):
            block = get_down_block(down_block_type=down_block_types[i],
        ...
        self.head = SISHeadAttnBlock(blk_in_channels,blk_in_channels,cls_count,self.time_emb_dim,attention_head_dim=attention_head_dim)
        # Decoder
        self.decoder = nn.ModuleList()
        decoder_channels = tuple(block_out_channels[::-1][1:])+(out_channels,)
        for i,blk_out_channels in enumerate(decoder_channels):
            block = get_up_block(up_block_type=up_block_types[i],
                                 in_channels=blk_in_channels,
                                 out_channels=blk_out_channels,
