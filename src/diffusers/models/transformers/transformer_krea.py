...
class KreaAttnProcessor:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError(
                "KreaAttnProcessor requires PyTorch 2.0. To use it, please upgrade PyTorch to version 2.0 or higher."
            )
...
class KreaAttnProcessor2_0:
    def __new__(cls, *args, **kwargs):
        deprecation_message = (
            "The KreaAttnProcessor2_0 class is deprecated and will be removed in a future version. "
            "Please use KreaAttnProcessor instead. "
        )
        deprecate("KreaAttnProcessor2_0", "1.0.0", deprecation_message, standard_warn=False)
        return KreaAttnProcessor(*args, **kwargs)


class KreaAttention(torch.nn.Module, AttentionModuleMixin):
    _default_processor_cls = KreaAttnProcessor
    _available_processors = [KreaAttnProcessor]
    ...
    def fuse_projections(self):
        ...
        if self.cross_attention_dim_head is None:
            ...
            with torch.device("meta"):
                self.qkv = nn.Linear(in_features, out_features, bias=True)
            self.qkv.load_state_dict(
            ...
            out_features, in_features = concatenated_weights.shape
            with torch.device("meta"):
                self.kv = nn.Linear(in_features, out_features, bias=True)
            self.kv.load_state_dict(
                {"weight": concatenated_weights, "bias": concatenated_bias}, strict=True, assign=True
            )
...
class KreaTimeTextImageEmbedding(nn.Module):
    def __init__(
        self,
        dim: int,
        time_freq_dim: int,
        ...
        if image_embed_dim is not None:
            self.image_embedder = KreaImageEmbedding(image_embed_dim, dim, pos_embed_seq_len=pos_embed_seq_len)
...
class KreaRotaryPosEmbed(nn.Module):
    def __init__(
        self,
...
class KreaTransformerBlock(nn.Module):
    def __init__(
        self,
        dim: int,
        ffn_dim: int,
...
class KreaTransformerModel(
    ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin, AttentionMixin
):
    r"""
    A Transformer model for video-like data used in the Krea model.
    ...
    def __init__(
        ...
        self.rope = KreaRotaryPosEmbed(attention_head_dim, patch_size, rope_max_seq_len)
        self.patch_embedding = nn.Conv3d(in_channels, inner_dim, kernel_size=patch_size, stride=patch_size)

        # 2. Condition embeddings
        # image_embedding_dim=1280 for I2V model
        self.condition_embedder = KreaTimeTextImageEmbedding(
            dim=inner_dim,
