...
class FluxWeightMappingHandler(WeightMappingHandler):
    """Flux-specific :class:`WeightMappingHandler` — original ↔ diffusers key conversion and variant detection."""

    _source_key_names = set(_FLUX_STATE_DICT_KEYS)
    _available_configs = dict(_FLUX_AVAILABLE_CONFIGS)
    _default_config = "flux-dev"
    _default_subfolder = "transformer"

    def map_to_diffusers(self, state_dict: dict[str, Any], **kwargs) -> dict[str, Any]:
        return _map_to_diffusers(state_dict, **kwargs)

    def map_from_diffusers(self, state_dict: dict[str, Any], **kwargs) -> dict[str, Any]:
        return _map_from_diffusers(state_dict, **kwargs)

    def detect_config(self, state_dict: dict[str, Any]) -> str | None:
        guidance_key = "guidance_in.in_layer.bias"
        x_embedder_key = "img_in.weight"

        if not self.is_source_format(state_dict):
            guidance_key = self.rename_key(guidance_key, FLUX_RENAME_PATTERNS)
            x_embedder_key = self.rename_key(x_embedder_key, FLUX_RENAME_PATTERNS)
...
FLUX_WEIGHT_MAPPING = FluxWeightMappingHandler()
