...
class FinalLayer(nn.Module):
    ...
    def forward(self, x, c=None, noise_mask=None, c_noisy=None, c_clean=None):
        if noise_mask is not None and c_noisy is not None and c_clean is not None:
            # Per-token modulation based on noise_mask
            batch_size, seq_len = x.shape[0], x.shape[1]
            scale_noisy = 1.0 + self.adaLN_modulation(c_noisy)  # (batch, hidden_size)
            scale_clean = 1.0 + self.adaLN_modulation(c_clean)  # (batch, hidden_size)
            
            # Select based on noise_mask
            noise_mask_expanded = noise_mask.unsqueeze(-1)  # (batch, seq_len, 1)
            scale = torch.where(noise_mask_expanded == 1,
...
class ZImageOmniTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        self.siglip_embedder = nn.Sequential(RMSNorm(siglip_feat_dim, eps=norm_eps), nn.Linear(siglip_feat_dim, dim, bias=True))
        ...
        for i in range(bsz):
            ...
            for j in range(len(size[i])):
                if size[i][j] is None:
                    ...
                    F, H, W = size[i][j]
                    ori_len = (F // pF) * (H // pH) * (W // pW)  # without padding
                    pad_len = (-ori_len) % SEQ_MULTI_OF
                    # assert ori_len + pad_len == unified_x.shape[0], f"Batch item {i}, patch {j}: ori_len {ori_len} + pad_len {pad_len} != unified_x.shape[0] {unified_x.shape[0]}"
                    x_item.append(einops.rearrange(
                        unified_x[cu_len:cu_len + ori_len].view(F // pF, H // pH, W // pW, pF, pH, pW, self.out_channels),
    ...
    def patchify_and_embed(
        ...
        for i in range(bsz):
            ...
            for j, cap_item in enumerate(all_cap_feats[i]):
                ...
                # cap_item_cu_len += 1 # for the padding tokens
                ...
                cap_item_cu_len += cap_item_ori_len # for the caption tokens
                cap_end_pos.append(cap_item_cu_len)
                cap_item_cu_len += 2 # for the image vae tokens and siglip tokens
            ...
            # print(f"> all_cap_feats[i]: {all_cap_feats[i].shape}", flush=True)
            ...
            for j, x_item in enumerate(all_x[i]):
                if x_item is not None:
                    ...
                    # print(f"x_item: {x_item.shape}", flush=True)
                    C, F, H, W = x_item.size()
                    x_size.append((F, H, W))
            ...
            # print(f"> all_x[i]: {all_x[i].shape}", flush=True)
            ...
            if all_siglip_feats[i] is None:
                ...
                # print(f"> all_siglip_feats[i]: {all_siglip_feats[i].shape}", flush=True)
        ...
        for i in range(bsz):
            ...
            assert all_x_padded[i].shape[0] + all_cap_padded_feats[i].shape[0] == sum(all_cap_len[i]) + sum(all_x_len[i]), f"Batch item {i}: x length {all_x_padded[i].shape[0]} + cap length {all_cap_padded_feats[i].shape[0]} != sum(all_cap_len[i]) + sum(all_x_len[i]) {sum(all_cap_len[i]) + sum(all_x_len[i])}"
    ...
    def forward(
        ...
        bsz = len(x)
        device = x[0].device
        t = torch.cat([t, torch.ones_like(t, dtype=t.dtype, device=device)], dim=0) # (N, D) -> (2N, D)
        # t = torch.cat([t, t], dim=0) # (N, D) -> (2N, D)
        t = t * self.t_scale
        t = self.t_embedder(t)  # (2N, embed_dim): first N are original t, last N are t=1
        
        # Split t into noisy and clean embeddings
        t_noisy = t[:bsz]  # (bsz, embed_dim) - original t
        t_clean = t[bsz:]  # (bsz, embed_dim) - t=1

        x = [cond_latents[i] + [x[i]] for i in range(bsz)]
        image_noise_mask = [[0] * (len(x[i]) - 1) + [1] for i in range(bsz)]
        # print(len(x[0]), len(cap_feats[0]), len(siglip_feats[0]), len(image_noise_mask[0]))
        # print([[_.shape, _.mean(), _.std()] for _ in x[0]])
        ...
        # Create x_noise_mask tensor matching x shape
        x_noise_mask_tensor = []
        for i in range(bsz):
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for layer in self.noise_refiner:
                x = self._gradient_checkpointing_func(layer, x, x_attn_mask, x_freqs_cis, 
                                                      noise_mask=x_noise_mask_tensor,
                                                      adaln_noisy=t_noisy_x, adaln_clean=t_clean_x)
        else:
            for layer in self.noise_refiner:
                x = layer(x, x_attn_mask, x_freqs_cis, 
                         noise_mask=x_noise_mask_tensor,
                         adaln_noisy=t_noisy_x, adaln_clean=t_clean_x)

        # cap embed & refine (no modulation, so no changes needed)
        ...
        if siglip_feats[0] is not None:
            for i in range(bsz):
                ...
                unified_freqs_cis.append(torch.cat([cap_freqs_cis[i][:cap_len], x_freqs_cis[i][:x_len], siglip_freqs_cis[i][:siglip_len]]))
                # Merge masks: cap_noise_mask + x_noise_mask + siglip_noise_mask
            ...
            for i in range(bsz):
                ...
                # Merge masks: cap_noise_mask + x_noise_mask
                unified_noise_mask.append(torch.tensor(
        ...
        # Create unified_noise_mask tensor matching unified shape
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            ...
            for layer in self.layers:
                # print(unified.shape, unified_noise_mask_tensor.shape)
                unified = layer(unified, unified_attn_mask, unified_freqs_cis,
                               noise_mask=unified_noise_mask_tensor,
                               adaln_noisy=t_noisy_x, adaln_clean=t_clean_x)
        ...
        unified = self.all_final_layer[f"{patch_size}-{f_patch_size}"](
            unified, 
            noise_mask=unified_noise_mask_tensor,
            c_noisy=t_noisy_x, c_clean=t_clean_x
        )
