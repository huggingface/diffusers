...
class NextDiT_3B_GQA_patch2_Adaln_Refiner_WHIT_CLIP(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        n_heads: int = 24,
        n_kv_heads: int = 8,
        axes_dims: Tuple[int, int, int] = (32, 32, 32),
        axes_lens: Tuple[int, int, int] = (1024, 512, 512),
        # 下列参数是你原 NextDiT/NextDiT_CLIP 里已有的 config 字段，
        # 这里作为可选项写进 ConfigMixin，方便从 config.json 加载。
        multiple_of: int = 256,
        ffn_dim_multiplier: Optional[float] = None,
        norm_eps: float = 1e-5,
        qk_norm: bool = False,
    ...
    def forward_with_cfg(
        ...
        cap_mask: torch.Tensor,
        cfg_scale: float,
        cfg_trunc: float = 100.0,
        renorm_cfg: bool = True,
        attn_bias: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> torch.Tensor:

        return self.inner_model.forward_with_cfg(
            x=x,
            t=t,
            cap_feats=cap_feats,
            cap_mask=cap_mask,
