# Copyright 2025 Ant Group, Alibaba Z-Image Team and The HuggingFace Team. All rights reserved.
#
...
#
# This is a modified version of transformer_z_image.py adapted for UniLLaDA.
# Key changes:
# - Optional flash_attn and dispatch_attention_fn imports
# - Modified attention mask handling for flash_attn compatibility
# - Renamed cap_embedder to semantic_embedder
# - Always create attention masks (no None case)
...
class ZSingleStreamAttnProcessor:
    ...
    def __call__(
        ...
        if attention_mask is not None and attention_mask.ndim == 2:
            if _HAS_FLASH_ATTN:
                # flash_attn: mask out inputs directly (matches original implementation)
        ...
        if _HAS_FLASH_ATTN:
            ...
            raise RuntimeError("Either flash_attn or dispatch_attention_fn is required.")
...
class LLaDA2UniImageTransformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    _skip_layerwise_casting_patterns = ["t_embedder", "semantic_embedder"]  # precision sensitive layers
    ...
    def _prepare_sequence(
        ...
        # Pad token — use index assignment to match original implementation exactly
        ...
        # Attention mask — always create to match original behavior
    ...
    def forward(
        ...
        cap_feats = self.semantic_embedder(torch.cat(cap_feats, dim=0))  # embed
