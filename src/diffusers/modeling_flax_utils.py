...
class FlaxModelMixin:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        pt_unexpected_keys = []
        pt_missing_keys = set(pt_model_dict.keys())
        ...
        for flax_key_tuple, flax_tensor in flax_state_dict.items():
            ...
            if flax_key in pt_model_dict:
                if flax_tensor.shape != pt_model_dict[flax_key].shape:
                    ...
                    pt_missing_keys.remove(flax_key)
            else:
                # weight is not expected by PyTorch model
                pt_unexpected_keys.append(flax_key)

        pt_model.load_state_dict(pt_model_dict)

        # re-transform pt_missing_keys to list
        pt_missing_keys = list(pt_missing_keys)

        if len(pt_unexpected_keys) > 0:
            logger.warning(
                "Some weights of the Flax model were not used when initializing the PyTorch model"
                f" {pt_model.__class__.__name__}: {pt_unexpected_keys}\n- This IS expected if you are initializing"
        ...
        if len(pt_missing_keys) > 0:
            logger.warning(
                f"Some weights of {pt_model.__class__.__name__} were not initialized from the Flax model and are newly"
                f" initialized: {pt_missing_keys}\nYou should probably TRAIN this model on a down-stream task to be able to"
