...
class AnimateDiffFreeNoiseMixin:
    ...
    def _check_inputs_free_noise(
        ...
        if negative_prompt is not None:
            if not isinstance(negative_prompt, (str, dict)):
                raise ValueError(f"Expected `negative_prompt` to have type `str` or `dict` but found {type(negative_prompt)=}")
        ...
        if max_frame >= num_frames:
            raise ValueError(f"The maximum frame index in `prompt` dict must be lesser than {num_frames=} and follow 0-based indexing.")
    ...
    def enable_free_noise(
        ...
        prompt_interpolation_callback: Optional[Callable[[DiffusionPipeline, int, int, torch.Tensor, torch.Tensor], torch.Tensor]] = None,
    ) -> None:
        r"""
