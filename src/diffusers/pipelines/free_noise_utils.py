...
from ..models.unets.unet_motion_model import CrossAttnDownBlockMotion, DownBlockMotion, FreeNoiseTransformerBlock, TransformerTemporalModel, UpBlockMotion
...
class AnimateDiffFreeNoiseMixin:
        ...
        for motion_module in block.motion_modules:
            ...
            for i in range(num_transformer_blocks):
                if isinstance(motion_module.transformer_blocks[i], FreeNoiseTransformerBlock):
                    motion_module.transformer_blocks[i].set_free_noise_properties(self._free_noise_context_length, self._free_noise_context_stride, self._free_noise_weighting_scheme)
                else:
    ...
    def enable_free_noise(self, context_length: Optional[int] = 16, context_stride: int = 4, weighting_scheme: str = "pyramid", shuffle: bool = True) -> None:
        r"""
        ...
        Args:
            context_length (`int`, defaults to `16`, *optional*):
                ...
                Motion Adapter was trained with (usually 16/24/32). If `None`, the default value from the motion adapter
                config is used.
            context_stride (`int`, *optional*):
                Long videos are generated by processing many frames. FreeNoise processes these frames in sliding windows of
                size `context_length`. Context stride allows you to specify how many frames to skip between each window.
                For example, a context length of 16 and context stride of 4 would process 24 frames as:
