...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import torch
        >>> from diffusers import MeissonicPipeline

        >>> pipe = MeissonicPipeline.from_pretrained("MeissonFlow/Meissonic", torch_dtype=torch.float16)
        >>> pipe = pipe.to("cuda")

        >>> prompt = "A white and blue coffee mug with a picture of a man on it."
