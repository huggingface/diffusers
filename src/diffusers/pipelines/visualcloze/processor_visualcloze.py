...
def resize_with_aspect_ratio(
    img: Image.Image, resolution: int, divisible: int = 16, aspect_ratio: Optional[float] = None
) -> Image.Image:
...
class VisualClozeProcessor:
    ...
    def process_image(
        self, input_images: List[List[Optional[str]]], vae_scale_factor: int
    ) -> Tuple[List[List[Image.Image]], List[List[List[int]]], List[int]]:
        ...
        if len({len(row) for row in input_images}) > 1:
        ...
        input_images = [
            [Image.open(img) if img is not None and isinstance(img, (str,)) else img for img in row]
            ...
            for j in range(0, n_task_images):
                if input_images[i][j] is not None:
                    resized = resize_with_aspect_ratio(
                        input_images[i][j], resolution, aspect_ratio=None, divisible=divisible
                    )
    ...
    def __call__(
        ...
        for i in range(len(task_prompt)):
            ...
            if upsampling:
                cur_processed_images, cur_image_size = self.process_image_upsampling(
                    input_images[i], height=height, width=width
                )
                cur_mask = self.process_mask_upsampling(cur_processed_images)
            else:
                cur_processed_images, cur_image_size, cur_target_position = self.process_image(
