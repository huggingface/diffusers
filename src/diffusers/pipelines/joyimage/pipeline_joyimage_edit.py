...
def _resize_center_crop(img: Image.Image, target_size: Tuple[int, int]) -> Image.Image:
    """Scale to cover target_size, then center-crop."""
...
def _dynamic_resize_from_bucket(image_size: Tuple[int, int],  basesize: int = 512) -> Tuple[int, int]:
    """
    Resize and center-crop an image to the nearest bucket dimensions.

    The best-matching bucket is selected based on the image's aspect ratio.
    The image is first scaled so that neither dimension is smaller than the
    target, then center-cropped to the exact target size.

    Args:
        image: Input PIL image. Returns None if None is passed.
        basesize: Base resolution used to generate candidate buckets.

    Returns:
        Resized and cropped PIL image, or None if input is None.
    """
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if height is not None and width is not None:
            # Override the image size if specified.
