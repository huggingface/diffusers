    ...
    def __call__(
        ...
        Args:
            ...
            mask (`List[PIL.Image.Image]`, *optional*):
                The input mask to denote which parts of the video should be used for conditioning and which parts
                should be generated. The general rule of thumb to keep in mind when preparing inputs for the VACE
                pipeline is that the input images, or frames of a video that you want to use for conditioning, should
                have a corresponding mask that is black in color. The black mask signifies that the model will not
                generate new content for that area, and only use those parts for conditioning the generation process.
                For parts/frames that should be generated by the model, the mask should be white in color. Currently,
                the pipeline only supports generating one video at a time. The mask should be a list of PIL images, a
            ...
            reference_images (`List[PIL.Image.Image]`, *optional*):
                ...
                character here. For a full list of supported tasks and use cases, it is recommended to refer:
                    - Diffusers-based minimal examples: https://github.com/huggingface/diffusers/pull/11582
                    - Original user-guide:
                      https://github.com/ali-vilab/VACE/blob/0897c6d055d7d9ea9e191dce763006664d9780f8/UserGuide.md
            conditioning_scale (`float`, `List[float]`, `torch.Tensor`, defaults to `1.0`):
                ...
                of control layers in the model:
                    - len(transformer.config.vace_layers)
