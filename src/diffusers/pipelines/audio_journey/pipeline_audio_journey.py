...
from . import AudioDiffusionPipelineOutput
...
class AudioJourneyPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromCkptMixin):
    r"""
    ...
    def __init__(
        ...
        self.encodec.dtype = unet.dtype
        
        self.channel_means = [  2.2741,  11.2872,  -3.3938,  -1.5556,  -0.0302,   7.6089,  -5.5797,
            ...
            -2.4755,   2.6202]
        self.channel_stds = [1.7524, 1.2040, 1.1098, 1.1021, 1.3688, 1.1374, 1.8660, 0.9791, 1.4331,
...
#         self.vae_scale_factor = 1
    ...
    def _rescale_encodec_batch(self, latents):
        latents = torch.Tensor.view(latents, [latents.shape[0], 128, 24, 21]).clone().to(self.device)
        
        mean_v = torch.tensor(self.channel_means).to(self.device).view(1, 128, 1, 1)
        
        std_v =torch.tensor(self.channel_stds).to(self.device).view(1, 128, 1, 1)
        
        latents = latents * std_v + mean_v
        
        latents = latents.view([latents.shape[0], 1, 128, 504])
        return latents
#     def _rescale_encodec_latent_single(self, latent):
#         image = torch.Tensor.view(latent, [128, 24, 21]).clone().to(self.device)
#         mean_v = torch.tensor(self.channel_means).to(self.device)
#         std_v =torch.tensor(self.channel_stds).to(self.device)
...
#         return image
    ...
    def to(
        ...
        super().to(torch_device = torch_device,
                torch_dtype = torch_dtype,
                silence_dtype_warnings = silence_dtype_warnings)
        self.encodec.to(torch_device, torch_dtype)
        self.encodec.device = torch_device
    ...
    def __call__(
        ...
        prompt_embeds, attn_mask = self._encode_prompt(
            prompt,
            device,
            num_images_per_prompt,
            do_classifier_free_guidance,
        ...
        latents = self._rescale_encodec_batch(latents)

        if not output_type == "latent":
            image = self.encodec.decoder(latents[:, 0, :, :])
            has_nsfw_concept = None
...
#         image = self.image_processor.postprocess(image, output_type=output_type, do_denormalize=do_denormalize)
        ...
        return AudioDiffusionPipelineOutput(images=image, nsfw_content_detected=has_nsfw_concept)
