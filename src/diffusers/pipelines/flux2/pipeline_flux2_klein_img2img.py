...
class Flux2KleinImg2ImgPipeline(DiffusionPipeline, Flux2LoraLoaderMixin):
    r"""
    ...
    def prepare_latents(
        ...
        # Klein's VAE wrapper automatically handles patchification and batch norm
    ...
    def check_inputs(
        ...
        if strength is not None and (strength <= 0 or strength > 1):
            raise ValueError(f"The value of strength should be in (0, 1], but is {strength}")
    ...
    def __call__(
        ...
        # 4. process images
        if image is None:
            raise ValueError("Img2Img pipeline requires an `image` input.")
        ...
        # duplicate image for batch size
        ...
        # Mathematically calculate sequence length since latents are not generated yet
        ...
        # Slice timesteps for Img2Img based on strength
        ...
        # Bypass the standard condition_images logic for standard Img2Img
        image_latents = None
