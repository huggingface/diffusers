...
- [`from_pretrained` method](https://github.com/huggingface/diffusers/blob/5cbed8e0d157f65d3ddc2420dfd09f2df630e978/src/diffusers/pipeline_utils.py#L139) that accepts a Hugging Face Hub repository id, *e.g.* [Lykon/dreamshaper-8](https://huggingface.co/Lykon/dreamshaper-8) or a path to a local directory, *e.g.*
"./stable-diffusion". To correctly retrieve which models and components should be loaded, one has to provide a `model_index.json` file, *e.g.* [Lykon/dreamshaper-8/model_index.json](https://huggingface.co/Lykon/dreamshaper-8/blob/main/model_index.json), which defines all components that should be
