...
class StableDiffusionImageVariationPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        if hasattr(unet.config, "sample_size") and unet.config.sample_size == 32:
            deprecation_message = (
                f"The configuration file of the unet has set the default `sample_size` to smaller than"
                f" 64 which seems highly unlikely .If you're checkpoint is a fine-tuned version of any of the"
                f" following: \n- CompVis/stable-diffusion-v1-4 \n- CompVis/stable-diffusion-v1-3 \n-"
                f" CompVis/stable-diffusion-v1-2 \n- CompVis/stable-diffusion-v1-1 \n- runwayml/stable-diffusion-v1-5"
                f" \n- runwayml/stable-diffusion-inpainting \n you should change 'sample_size' to 64 in the"
                f" configuration file. Please make sure to update the config accordingly as leaving `sample_size=32`"
                f" in the config might lead to incorrect results in future versions. If you have downloaded this"
                f" checkpoint from the Hugging Face Hub, it would be very nice if you could open a Pull request for"
                f" the `unet/config.json` file"
            )
