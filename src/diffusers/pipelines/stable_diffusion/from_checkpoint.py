...
# modified convert_from_ckpt to remove pipeline imports
# pipeline class is passed as parameter to conversion function
# avoids circular import error when using code for from_checkpoint class method
...
def convert_ldm_clip_checkpoint(checkpoint):
    ...
    # use pretrained weights if loading from pruned checkpoint (i.e. missing text encoder keys)
    if text_model_dict:
...
def load_pipeline_from_original_stable_diffusion_ckpt(
    ...
    **kwargs,
    ...
    :param pipeline_class: Class of the pipeline instance to be instantiated
    ...
    :param requires_safety_checker: Whether to use safety checker. If True, automatically
    downloads safety checker weights.
    :param torch_dtype: The torch datatype that pipeline weights should be sent to. If `None`, pipeline weights will
    have same datatype as the original checkpoint file
    """
    if from_safetensors is None:
        from_safetensors = checkpoint_path.rsplit(".", 1)[-1] == "safetensors"
    ...
    if not os.path.isfile(checkpoint_path):
        ...
        repo_id = str(Path().joinpath(*checkpoint_path.parts[:2]))
        file_path = str(Path().joinpath(*checkpoint_path.parts[2:]))

        user_agent = {"pipeline_class": pipeline_name}
        ...
        checkpoint_path = Path(cached_folder) / file_path
        ...
        if original_config_file is None:
            ...
            if key_name in checkpoint and checkpoint[key_name].shape[-1] == 1024:
                if not os.path.isfile("v2-inference-v.yaml"):
                    ...
                    with open(original_config_file, "wb") as fp:
                        fp.write(r.content)
    ...
    if pipeline_name == "PaintByExamplePipeline":
        ...
        raise (f"Checkpoint conversion does not support pipeline class {pipeline_name}")

    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    return pipe.to(device, dtype=torch_dtype)
