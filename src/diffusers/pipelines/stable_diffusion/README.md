...
download the weights with `git lfs install; git clone https://huggingface.co/runwayml/stable-diffusion-v-1-5` and instead pass the local path to the cloned folder to `from_pretrained` as shown below.
...
pipeline = DiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v-1-5")
```
...
```
git lfs install
git clone https://huggingface.co/runwayml/stable-diffusion-v-1-5
```
...
pipe = StableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v-1-5")
pipe = pipe.to("cuda")
