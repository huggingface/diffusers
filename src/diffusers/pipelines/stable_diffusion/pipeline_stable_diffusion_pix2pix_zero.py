...
class StableDiffusionPix2PixZeroPipeline(DiffusionPipeline):
    r"""
    Pipeline for pixel-level image editing with Stable Diffusion models using Pix2Pix Zero.
    ...
    def __call__(
        ...
        Args:
            ...
            source_embeds (`torch.Tensor`):
                Source concept embeddings used in discovering the edit direction.
            target_embeds (`torch.Tensor`):
                Target concept embeddings used in discovering the edit direction.
    ...
    def invert(
        ...
        Args:
            ...
            image (`torch.FloatTensor` `np.ndarray`, `PIL.Image.Image`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                `Image` or tensor representing an image batch to be used for conditioning. Can also accept image
                latents as `image`, and if passing latents directly they are not encoded again.
            num_inference_steps (`int`, *optional*, defaults to 50):
                The number of denoising steps. More denoising steps usually lead to a higher quality image at the
                expense of slower inference.
        ...
        Returns:
            ...
            `tuple`:
                ...
                [`~pipelines.stable_diffusion.pipeline_stable_diffusion_pix2pix_zero.Pix2PixInversionPipelineOutput`]
                is returned, otherwise a `tuple` is returned where the first element is the inverted latents tensor and
                then second is the corresponding decoded image.
