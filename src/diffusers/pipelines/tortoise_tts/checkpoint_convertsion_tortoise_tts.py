...
attn = AttentionBlock(channels=1024, num_heads=16, num_head_channels=-1, relative_pos_embeddings=True)
...
attn_hf = TortoiseTTSSelfAttention(
    query_dim=1024,
    n_heads=16,
    dim_head=64,
    dropout=0.0,
...
# init weights
attn_hf.state_dict()["SelfAttention.q.weight"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.q.bias"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.k.weight"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.k.bias"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.v.weight"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.v.bias"].copy_(
    torch.concatenate(
        [
...
attn_hf.state_dict()["SelfAttention.relative_attention_bias.weight"].copy_(
    dict(attn.named_parameters())["relative_pos_embeddings.relative_attention_bias.weight"]
)
