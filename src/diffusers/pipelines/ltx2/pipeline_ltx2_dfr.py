...
class LTX2DFRPipeline(DiffusionPipeline, FromSingleFileMixin, LTX2LoraLoaderMixin):
    ...
    def _inference_timesteps(self, sigmas: list[float], device: torch.device) -> torch.Tensor:
        ...
        [`FlowMatchEulerDiscreteScheduler`] takes distilled sigmas *without* a terminal 0 (it appends one) and exposes
        `len(sigmas)` timesteps. [`LTXEulerAncestralRFScheduler`] takes distilled sigmas *plus* terminal 0 and the loop
        must skip the last timestep, matching [`LTXI2VLongMultiPromptPipeline`].
        """
    ...
    def denoise(
        ...
        recovers the blended `denoised`. Stage 1 / 2 / the epilogue keep [`FlowMatchEulerDiscreteScheduler`] and take
    ...
    def __call__(
        ...
        Args:
            ...
            audio_latents (`torch.Tensor`, *optional*):
                Raw unpacked `(batch_size, channels, length, mel_bins)` audio latents. Stage 2 still runs a joint audio
                pass; the shipped waveform of a composed recipe is stage 1's, which the caller keeps.
            ...
            video_tiles (`list[LTX2DFREpilogueTile]`, *optional*):
                Epilogue tiling from [`~diffusers.pipelines.ltx2.dfr_layout.epilogue_tiles`], so a resolution too large
                for one forward pass can still step a single canvas: each step runs the transformer once per tile and
                blends the predictions. Resolved into a token plan against this pass's own RoPE coordinates, which is
                why the layout is passed rather than the plan — the coordinates only exist once
