...
class LTX2DFRTemporalRefinePipeline(DiffusionPipeline, FromSingleFileMixin, LTX2LoraLoaderMixin):
    r"""
    ...
    on [`FlowMatchEulerDiscreteScheduler`] via [`LTX2DFRPipeline`]. Call this pipeline once per round; the caller loops
    for 2x / 4x.
    ...
    Args:
        scheduler ([`LTXEulerAncestralRFScheduler`]):
            Ancestral Euler scheduler in the rectified-flow parameterization. Construct with `eta=0.5` to match the DFR
            temporal recipe.
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline._maybe_normalize_video_latents
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline._inference_timesteps
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline.encode_prompt
    def encode_prompt(
        self,
        prompt: str | list[str],
        num_videos_per_prompt: int = 1,
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline.prepare_latents
    def prepare_latents(
        self,
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline.encode_conditions
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline.denoise
    def denoise(
        self,
        latents: torch.Tensor,
    ...
    # Copied from diffusers.pipelines.ltx2.pipeline_ltx2_dfr.LTX2DFRPipeline._public_audio_from_packed
    ...
    def __call__(
        ...
        Args:
            ...
            generator (`torch.Generator` or `list[torch.Generator]`, *optional*):
                Random generator(s) for reproducibility. Ancestral draws use a separate per-tile seed derived from this
                generator's `initial_seed()`.
            prompt_embeds (`torch.Tensor`, *optional*):
        ...
        # The canvas keeps its padding (a slot on the pad must survive into the next stage), so both the latent
        # output and a direct decode cover the full padded grid; `trim_canvas` before a standalone decode is the
        # caller's job. `video_latents` / `carry_keyframes` are VAE-normalized here; the public contract is raw.
        ...
        if output_type == "latent":
            ...
            audio_samples = min(
                audio.shape[-1], round(canvas_frames / playback_fps * self.vocoder.config.output_sampling_rate)
            )
