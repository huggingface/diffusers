    ...
    def __call__(
        ...
        Args:
            ...
            strength (`float`, *optional*, defaults to 0.8):
                Conceptually, indicates how much to transform the reference image embedding. Must be between 0 and 1.
                `image` will be used as a starting point, adding more noise to it the larger the `strength`. The number
                of denoising steps depends on the amount of noise initially added.
            image (`PIL.Image.Image`, `List[PIL.Image.Image]`, `torch.FloatTensor`, or `List[torch.FloatTensor]`):
                ...
                tensor, it will first be encoded into image embedding using `image_encoder`. If passing image embedding
                directly, it will not be encoded again.
            negative_prompt (`str` or `List[str]`, *optional*):
