...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    """Pass either ``num_inference_steps`` (uniform), ``timesteps`` (explicit) or ``sigmas`` (explicit).

    Returns a ``(timesteps, num_inference_steps)`` tuple. Additional kwargs (e.g. ``mu`` for FlowMatch schedulers) are
    forwarded to ``scheduler.set_timesteps``.
    """
