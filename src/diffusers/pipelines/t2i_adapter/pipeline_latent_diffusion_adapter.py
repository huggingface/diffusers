...
class LDMClasstoImageAdapterPipeline(DiffusionPipeline, StableDiffusionMixin):
    r"""
    ...
    model_cpu_offload_seq = "class_embedder->adapter->unet->vae"
    ...
    def encode_prompt(        
        ...
        if prompt_embeds is None:
            prompt_embeds = self.class_embedder(torch.tensor(prompt).to(device))

        if self.class_embedder is not None:
            prompt_embeds_dtype = self.class_embedder.dtype
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:        
            negative_prompt_embeds = self.class_embedder(torch.tensor(batch_size * [self.class_embedder.n_classes - 1]).to(device))
    ...
    def check_inputs(
        ...
        elif prompt is not None and (not isinstance(prompt, int) and not isinstance(prompt, list)):
            raise ValueError(f"`prompt` has to be of type `int` or `list` but is {type(prompt)}")
    ...
    def __call__(
        ...
        self.check_inputs(
            prompt, image, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds
        )
...
class LDMTexttoImageAdapterPipeline(DiffusionPipeline, StableDiffusionMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion augmented with T2I-Adapter
    https://arxiv.org/abs/2302.08453
