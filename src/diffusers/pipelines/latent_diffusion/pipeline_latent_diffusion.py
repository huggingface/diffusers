...
class LDMClassToImagePipeline(DiffusionPipeline):
    r"""
    Pipeline for class-to-image generation using latent diffusion (ImageNet condtional generation).
    ...
    model_cpu_offload_seq = "class_embeder->unet->vqvae"
    ...
    def __init__(
        ...
        super().__init__()
        self.register_modules(vqvae=vqvae, class_embedder=class_embedder, unet=unet, scheduler=scheduler)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if guidance_scale != 1.0:            
            negative_prompt_embeds = self.class_embedder(torch.tensor(batch_size * [self.class_embedder.n_classes - 1]).to(self._execution_device))
...
class LDMClassEmbedderConfig(PretrainedConfig):
    model_type = "ldmclsemb"
