...
EXAMPLE_DOC_STRING = """
    Examples:
        ```python
        >>> import torch
        >>> from diffusers import MagiImageToVideoPipeline
        >>> from diffusers.utils import export_to_video, load_image

        >>> pipeline = MagiImageToVideoPipeline.from_pretrained("sand-ai/MAGI-1-4.5B", torch_dtype=torch.float16)
        >>> pipeline = pipeline.to("cuda")

        >>> image = load_image("path/to/image.jpg")
        >>> prompt = "A cat playing in a garden. The cat is chasing a butterfly."
        >>> output = pipeline(prompt=prompt, image=image, num_frames=24, height=720, width=720).frames[0]
        >>> export_to_video(output, "magi_i2v_output.mp4", fps=8)
        ```
"""


class MagiImageToVideoPipeline(DiffusionPipeline):
    r"""
    Pipeline for image-to-video generation using MAGI-1.
    ...
    def __init__(
        ...
        self.vae_scale_factor_spatial = 2 ** (3 if hasattr(self.vae, "config") else 8)  # Default to 8 for 3 downsamples
    ...
    def prepare_image_latents(
        ...
        Returns:
            `torch.Tensor`: Encoded image latents.
        """
        # Convert PIL image to tensor
        if isinstance(image, PIL.Image.Image):
            image = self.video_processor.preprocess_image(image)
    ...
    def _prepare_image_based_latents(
        self,
        batch_size: int,
        ...
        """
        Prepare latents for diffusion with image conditioning.
        ...
        Args:
            ...
            image_latents (`torch.Tensor`, *optional*): Image latents for conditioning.
        ...
        # If we have image latents, use them to condition the first frame
        if image_latents is not None:
            # Expand image latents to match the temporal dimension of the first frame
            image_latents = image_latents.unsqueeze(2)  # [B, C, 1, H, W]

            # Only replace the first frame with the image latents
            latents[:, :, 0, :, :] = image_latents.squeeze(2)
    ...
    def __call__(
        ...
        Args:
            ...
            image (`torch.Tensor` or `PIL.Image.Image`):
                The input image to guide the video generation.
        ...
        # 6. Prepare latent variables with image conditioning
        num_channels_latents = self.transformer.in_channels
        latents = self._prepare_image_based_latents(
