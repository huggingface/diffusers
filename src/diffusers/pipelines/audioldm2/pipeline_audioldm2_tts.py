...
class AudioLDM2TTSPipeline(DiffusionPipeline):
    r"""
    Pipeline for text-to-speech generation using AudioLDM2.
    ...
    Args:
        ...
        text_encoder_2 ([`~transformers.VitsModel`]):
        ...
        tokenizer_2 ([`~transformers.VitsTokenizer`]):
    ...
    def __init__(
        ...
        text_encoder_2: VitsModel,
        projection_model: AudioLDM2ProjectionModel,
        language_model: GPT2Model,
        tokenizer: Union[RobertaTokenizer, RobertaTokenizerFast],
        tokenizer_2: VitsTokenizer,
        feature_extractor: ClapFeatureExtractor,
        unet: AudioLDM2UNet2DConditionModel,
        scheduler: KarrasDiffusionSchedulers,
        vocoder: SpeechT5HifiGan,
        learnable_position_embedding: AudioLDMLearnablePositionalEmbedding,
    ):
        super().__init__()
        ...
        self.register_modules(
            ...
            scheduler=scheduler,
            vocoder=vocoder,
            learnable_position_embedding=learnable_position_embedding,
        )
    ...
    def text_to_phoneme(self, transcription):
        """
        Preprocess and convert text to phonemes
        """
        transcription = re.sub(r"<.*?>", "", transcription)
        transcription = unidecode(transcription)
        transcription = transcription.lower()
        phonemes = phonemize(
            transcription, language="en-us", backend="espeak", strip=True, preserve_punctuation=True, with_stress=True
        )
        phonemes = re.sub(re.compile(r"\s+"), " ", phonemes)
        phonemes += "⚠"  # The last token in offical audioldm2
        ...
        if prompt_embeds is None:
            ...
            for tokenizer, text_encoder in zip(tokenizers, text_encoders):
                if text_encoder.config.model_type == "clap":
                    ...
                    phonemes = self.text_to_phoneme(transcription)
                    text_inputs = self.tokenizer_2(phonemes, return_tensors="pt", padding="max_length")
                    text_input_ids = text_inputs.input_ids
                    attention_mask = text_inputs.attention_mask
                ...
                if text_encoder.config.model_type == "clap":
                    ...
                    prompt_embeds = self.learnable_position_embedding(prompt_embeds)
            ...
            for tokenizer, text_encoder in zip(tokenizers, text_encoders):
                ...
                if text_encoder.config.model_type == "clap":
                    ...
                    negative_prompt_embeds = torch.zeros(
                        ...
                        self.text_encoder_2.text_encoder.config.hidden_size,
                    ).to(dtype=self.text_encoder_2.dtype, device=device)
                    negative_attention_mask = torch.zeros(batch_size, self.tokenizer_2.model_max_length).to(
