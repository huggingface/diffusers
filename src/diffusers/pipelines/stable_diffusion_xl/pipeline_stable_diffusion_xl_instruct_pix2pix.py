...
class StableDiffusionXLInstructPix2PixPipeline(
    ...
    # Copied from diffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_instruct_pix2pix.StableDiffusionInstructPix2PixPipeline.prepare_ip_adapter_image_embeds
    ...
    def __call__(
        ...
        Args:
            ...
            ip_adapter_image_embeds (`list[torch.Tensor]`, *optional*):
                ...
                IP-adapters. Each element should be a tensor of shape `(batch_size, num_images, emb_dim)`. Because this
                pipeline runs a three-way classifier-free guidance batch, each tensor should contain the image
                embedding followed by two copies of the negative image embedding if `do_classifier_free_guidance` is
                set to `True`. If not provided, embeddings are computed from the `ip_adapter_image` input argument.
        ...
        # 3.1 Encode ip_adapter_image. Like `prompt_embeds`, the embeddings are ordered
        # [image_embeds, negative_image_embeds, negative_image_embeds] to match the 3-way pix2pix batch.
        image_embeds = None
