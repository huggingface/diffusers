...
def prepare_unet(unet: UNet2DConditionModel):
    ...
    for name in unet.attn_processors.keys():
        ...
        pix2pix_zero_attn_procs[name] = (
            Pix2PixZeroAttnProcessor2_0() if hasattr(F, "scaled_dot_product_attention") else Pix2PixZeroAttnProcessor()
        )
        if "attn2" in name:
...
class Pix2PixZeroAttnProcessor:
    ...
    def __call__(
        ...
        if is_cross and timestep is not None:
...
class Pix2PixZeroAttnProcessor2_0:
    """An attention processor class to store the attention weights.
    In Pix2Pix Zero, it happens during computations in the cross-attention blocks."""
