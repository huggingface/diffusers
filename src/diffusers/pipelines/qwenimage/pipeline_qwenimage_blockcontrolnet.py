...
from ...models import AutoencoderKLQwenImage, QwenImageBlockTransformer2DModel
from ...models.controlnets.controlnet_qwenimage_blockwise import QwenImageBlockwiseMultiControlNetModel
...
EXAMPLE_DOC_STRING = """
   Examples:
        ...
        >>> from diffusers import QwenImage_Blockwise_Controlnet, QwenImageBlockwiseMultiControlNetModel, QwenImageBlockControlNetPipeline

        >>> # QwenImageControlNetModel
        >>> controlnet =QwenImage_Blockwise_Controlnet.from_pretrained(
        ...     "SahilCarterr/Qwen-Image-Blockwise-ControlNet-Depth", torch_dtype=torch.bfloat16
        ... )

        >>> controlnet = QwenImageBlockwiseMultiControlNetModel([controlnet])
        ...
        ...     true_cfg_scale=4.0,
        ... ).images[0]
        >>> image.save("qwenimage_cn_block_multi.png")
        ```
"""
...
class QwenImageBlockControlNetPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    r"""
    ...
    def __init__(
        ...
        transformer: QwenImageBlockTransformer2DModel,
        controlnet: List[QwenImageBlockwiseMultiControlNetModel],
    ):
        super().__init__()
    ...
    def check_inputs(
        ...
        if max_sequence_length is not None and max_sequence_length > 1024:
            raise ValueError(f"`max_sequence_length` greater than 1024 led to unpredictable behaviour but is {max_sequence_length}")
    ...
    def __call__(
        ...
        if not isinstance(control_guidance_start, list) and isinstance(control_guidance_end, list): # if control_guidance_start is scalar
            control_guidance_start = len(control_guidance_end) * [control_guidance_start]
        elif not isinstance(control_guidance_end, list) and isinstance(control_guidance_start, list): # if control_guidance_end is scalar
            control_guidance_end = len(control_guidance_start) * [control_guidance_end]
        elif not isinstance(control_guidance_start, list) and not isinstance(control_guidance_end, list): # both are list, create list for multi controlnet
            mult = len(control_image) if isinstance(self.controlnet, QwenImageBlockwiseMultiControlNetModel) else 1
        ...
        if isinstance(self.controlnet, QwenImageBlockwiseMultiControlNetModel):
            control_images = []
            for control_image_ in control_image:
                ...
                control_image_ = control_image_.squeeze(2)
        ...
        sigma_min = 0
        sigma_max = 1
        sigma_start = sigma_min + (sigma_max - sigma_min) * strength
        sigmas = np.linspace(sigma_start, sigma_min, num_inference_steps) if sigmas is None else sigmas
        ...
        controlnet_keep = []
        for progress_id, timestep in enumerate(tqdm(timesteps)):
