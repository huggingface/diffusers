...
class OnnxStableDiffusion3Pipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        text_encoder_3: OnnxRuntimeModel,
        tokenizer_3: T5TokenizerFast,
        transformer: OnnxRuntimeModel,
    ...
    def _get_t5_prompt_embeds(
        ...
        prompt_embeds = self.text_encoder_3(input_ids=text_input_ids.astype(np.int32))[0]
    ...
    def _get_clip_prompt_embeds(
        ...
        prompt_embeds = text_encoder(input_ids=text_input_ids.astype(np.int32))
        pooled_prompt_embeds = prompt_embeds[0]

        if clip_skip is None:
            prompt_embeds = prompt_embeds[-2]
        else:
            prompt_embeds = prompt_embeds[-(clip_skip + 2)]
        ...
        pooled_prompt_embeds = np.tile(pooled_prompt_embeds, (1, num_images_per_prompt, 1))
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            clip_prompt_embeds = np.concatenate([prompt_embed, prompt_2_embed], axis=-1)
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_clip_prompt_embeds = np.concatenate([negative_prompt_embed, negative_prompt_2_embed], axis=-1)
    ...
    def __call__(
        ...
        if self.do_classifier_free_guidance:
            ...
            pooled_prompt_embeds = np.concatenate([negative_pooled_prompt_embeds, pooled_prompt_embeds], axis=0)
        ...
        with self.progress_bar(self.scheduler.timesteps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # perform guidance
                if self.do_classifier_free_guidance:
                    noise_pred_uncond, noise_pred_text = np.array_split(noise_pred, 2, axis=0)
                ...
                latents = scheduler_output.numpy()
