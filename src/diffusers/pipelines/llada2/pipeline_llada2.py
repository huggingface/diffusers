    ...
    def __call__(
        ...
        Returns:
            [`~pipelines.llada2.pipeline_llada2.LLaDA2PipelineOutput`] or `tuple`:
                ...
                otherwise a `tuple` is returned where the first element is the generated token IDs
                (`torch.LongTensor`) and the second element is the decoded texts (`list[str]`), or `None` when
                `output_type` is `"seq"`.
        """
