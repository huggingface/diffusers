...
class AltDiffusionImg2ImgPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    r"""
    Pipeline for text-guided image-to-image generation using AltDiffusion.
    ...
    Args:
        ...
        tokenizer (`XLMRobertaTokenizer`):
            A [`~transformers.XLMRobertaTokenizer`] to tokenize text.
    ...
    def enable_sequential_cpu_offload(self, gpu_id=0):
        r"""
        Offload all models to CPU to significantly reduce memory usage. The state dicts of the `unet`, `text_encoder`,
        `vae` and `safety_checker` are saved to CPU and then moved to a `torch.device('meta') and loaded to GPU only
        when their specific submodule `forward` method is called. Offloading happens on a submodule basis and memory
        savings are higher than using `enable_model_cpu_offload` at the cost of lower performance.
        """
    ...
    def __call__(
        ...
        Args:
            ...
            image (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, or `List[np.ndarray]`):
                `Image`, or tensor representing an image batch that is used as the starting point for the process. Can
                also accpet image latents as `image` if passing latents directly and it will not be encoded again.
            strength (`float`, *optional*, defaults to 0.8):
                Indicates the extent to transform the reference `image`. Must be between 0 and 1. `image` is used as a
                starting point, and more noise is added the higher the `strength`. The number of denoising steps
            ...
            return_dict (`bool`, *optional*, defaults to `True`):
                Whether or not to return a [`~pipelines.alt_diffusion.AltDiffusionPipelineOutput`] instead of a plain
                tuple.
