...
class AltDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        if hasattr(unet.config, "sample_size") and unet.config.sample_size == 32:
            deprecation_message = (
                f"The configuration file of this unet: {unet} has set the default `sample_size` to smaller than 64"
                " which seems highly unlikely .If you're checkpoint is a fine-tuned version of any of the following:"
