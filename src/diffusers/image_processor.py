...
class SevaImageProcessor(VaeImageProcessor):
    """
    Image processor for SEVA image resize and crop.
    ...
    Args:
        ...
        do_normalize (`bool`, *optional*, defaults to `True`):
            Whether to normalize the image to [-1,1].
        do_binarize (`bool`, *optional*, defaults to be `False`):
            Whether to binarize the image to 0/1.
        do_convert_rgb (`bool`, *optional*, defaults to be `False`):
            Whether to convert the images to RGB format.
        do_convert_grayscale (`bool`, *optional*, defaults to be `False`):
    ...
    def transform_cam_intrinsics(
        ...
        if resize_mode == "default":
            ...
            factor = self.get_resizing_factor(
                (height, width),
                (current_h, current_w),
                cover_target=resize_mode != "pad",
            )
            resized_h, resized_w = [int(np.ceil(factor * size)) for size in (current_h, current_w)]

        

        rx_center, ry_center = resized_w // 2, resized_h // 2
        tx_center, ty_center = target_w // 2, target_h // 2
        if resize_mode != "pad":
            crop_top = max(0, ry_center - ty_center)
            crop_left = max(0, rx_center - tx_center)
            image = image[:, :, crop_top:crop_top+height, crop_left:crop_left+width]
            pad_left, pad_top = 0, 0
        else:
            pad_top = max(0, ty_center - ry_center)
            pad_left = max(0, tx_center - rx_center)
            pad_bottom = max(0, target_h - pad_top - image.shape[-2])
            pad_right = max(0, target_w - pad_left - image.shape[-1])
            image = F.pad(
                image, 
