...
"""Unit tests for PingPongScheduler."""
...
def _scheduler(n=8, logsnr_min=-6.2, logsnr_max=2.0):
    s = PingPongScheduler(num_inference_steps=n, logsnr_min=logsnr_min, logsnr_max=logsnr_max)
    s.set_timesteps()
    return s


@require_torch
class TestPingPongScheduler(unittest.TestCase):
    # ------------------------------------------------------------------
    def test_set_timesteps_shapes(self):
        s = _scheduler(n=8)
        self.assertEqual(s.sigmas.shape, (9,))  # N+1
        self.assertEqual(s.timesteps.shape, (8,))  # N
    ...
    def test_full_loop_no_nan(self):
        """Run 8 steps of sampling without producing NaN or Inf."""
        s = _scheduler()
    ...
    def test_set_timesteps_override(self):
        s = _scheduler(n=8)
        s.set_timesteps(4)
        self.assertEqual(s.num_inference_steps, 4)
        self.assertEqual(s.sigmas.shape, (5,))
        self.assertEqual(s.timesteps.shape, (4,))
    ...
    def test_add_noise_linear_interpolation(self):
        ...
        # sample 0: t=0.5, eps=0 → (1-0.5)*1 + 0.5*0 = 0.5
    ...
    # ------------------------------------------------------------------
    def test_len(self):
        s = PingPongScheduler(num_inference_steps=4)
        self.assertEqual(len(s), 4)

    # ------------------------------------------------------------------
    def test_config_roundtrip(self):
        s = PingPongScheduler(num_inference_steps=4, logsnr_min=-5.0, logsnr_max=1.5)
        with tempfile.TemporaryDirectory() as d:
            s.save_pretrained(d)
            s2 = PingPongScheduler.from_pretrained(d)
        self.assertEqual(s2.config.num_inference_steps, 4)
    ...
    def test_deterministic_with_generator(self):
        ...
        v = torch.randn(1, 4, 8)
        # Use two independent schedulers so _step_index=0 for both calls.
        s1 = _scheduler()
        out1 = s1.step(v, s1.timesteps[0], sample, generator=torch.Generator().manual_seed(99)).prev_sample
        s2 = _scheduler()
        out2 = s2.step(v, s2.timesteps[0], sample, generator=torch.Generator().manual_seed(99)).prev_sample
        self.assertTrue(torch.allclose(out1, out2))

    # ------------------------------------------------------------------
    def test_stochastic_renoise_differs_across_seeds(self):
        """Different RNG seeds must yield different prev_sample (re-noise IS stochastic)."""
        sample = torch.randn(1, 4, 8)
        v = torch.randn(1, 4, 8)
        s1 = _scheduler()
    ...
    def test_step_index_advances(self):
        """_step_index increments by 1 per step() call."""
        s = _scheduler()
        sample = torch.randn(2, 4, 8)
        v = torch.randn(2, 4, 8)
    ...
    def test_set_timesteps_resets_step_index(self):
        """Calling set_timesteps again must reset the counter."""
        s = _scheduler()
        sample = torch.randn(1, 4, 8)
        v = torch.randn(1, 4, 8)
        s.step(v, s.timesteps[0], sample)
