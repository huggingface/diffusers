...
class SanaPipelineTesterConfig(BasePipelineTesterConfig):
...
class TestSanaPipeline(SanaPipelineTesterConfig, PipelineTesterMixin):
    ...
    # The `AutoencoderDC` VAE's multiscale linear attention runs a depthwise `Conv2d`, which some cuDNN builds
    # have no bfloat16 engine for. The decorators below repeat the ones the base method is declared with:
    # overriding a test drops the marks it inherited.
...
class TestSanaPipelineMemory(SanaPipelineTesterConfig, MemoryTesterMixin):
    """Memory optimization tests (CPU offload, group offload, layerwise casting) for the Sana pipeline."""
...
@slow
@require_torch_accelerator
class TestSanaPipelineIntegration:
    ...
    def test_sana_1024(self):
        ...
        assert np.allclose(output_slice, expected_slice, atol=1e-4)
