...
class SanaSprintImg2ImgPipelineTesterConfig(BasePipelineTesterConfig):
    pipeline_class = SanaSprintImg2ImgPipeline
    # Sana Sprint is a timestep-distilled model: it takes no negative prompt.
    required_input_params_in_call_signature = frozenset(
        ["prompt", "image", "height", "width", "guidance_scale", "prompt_embeds"]
    )
    ...
    def get_dummy_inputs(self):
        ...
        image = randn_tensor((1, 3, 32, 32), generator=generator, device=torch.device("cpu"))
        return {
            "prompt": "",
            "image": image,
            "strength": 0.5,
...
class TestSanaSprintImg2ImgPipeline(SanaSprintImg2ImgPipelineTesterConfig, PipelineTesterMixin):
    ...
    # The pre-migration test compared the *signed* max of the difference, which reads 0.19 here; the
    # actual largest deviation is ~0.23, hence the tolerance above the 0.2 used elsewhere.
    ...
    # `SanaSprintImg2ImgPipeline` recomputes `latents` from `denoised` after the denoising loop
...
class TestSanaSprintImg2ImgPipelineMemory(SanaSprintImg2ImgPipelineTesterConfig, MemoryTesterMixin):
    """Memory optimization tests (CPU offload, group offload, layerwise casting) for the Sana Sprint img2img pipeline."""
