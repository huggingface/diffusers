    ...
    def test_stable_diffusion_intermediate_state(self):
        ...
        def callback_fn(step: int, timestep: int, latents: torch.FloatTensor) -> None:
            ...
            if step == 1:
                ...
                expected_slice = np.array([-0.5713, -0.3018, -0.9814,  0.0466, -0.8790,  0.7600, -1.7340,  0.1044, 1.1610], dtype=torch.float64)
                ...
                from diffusers.utils.testing_utils import print_tensor_test; print_tensor_test(expected_slice); assert np.abs(latents_slice.flatten() - expected_slice).max() < 1e-2
