...
from diffusers import AutoencoderKLCogVideoX, CogVideoXFunInpaintPipeline, CogVideoXTransformer3DModel, DDIMScheduler
...
class CogVideoXFunInpaintPipelineFastTests(PipelineTesterMixin, unittest.TestCase):
    pipeline_class = CogVideoXFunInpaintPipeline
    params = TEXT_TO_IMAGE_PARAMS - {"cross_attention_kwargs"}
    batch_params = TEXT_TO_IMAGE_BATCH_PARAMS.union({"video", "mask_video"})
    ...
    def get_dummy_inputs(self, device, seed: int = 0, num_frames: int = 8):
        ...
        video = [Image.new("RGB", (width, height))] * num_frames
        mask_video = [Image.new("L", (width, height), 255) for _ in range(num_frames)]
        mask_video[0] = Image.new("L", (width, height), 0)
