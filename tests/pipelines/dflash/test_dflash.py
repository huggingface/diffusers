...
from diffusers import DFlashPipeline, DFlashTokenDiffusionScheduler
...
class DFlashRegressionTest(unittest.TestCase):
    """Pin the bug patterns surfaced in https://github.com/huggingface/diffusers/issues/13598
    (LLaDA2 review) for any that are relevant to DFlash.

    DFlash is batch_size=1 only and does not pass an `attention_mask` to the target model, so
    issues #1 (padding mask), #2 (block_length scheduler routing), and #5 (batched EOS row freeze)
    don't apply. The applicable patterns are #3 (callback keys must resolve), #4 (EOS at the first
    generated position), and #6 (progress-bar config must not be mutated by `__call__`).
    """
    ...
    def test_stop_token_at_first_generated_position_triggers_stop(self):
        """Issue #4 analogue: a stop token at index `num_input_tokens` (the first generated
        position) must terminate generation. Verified at the scheduler level — `check_should_stop`
        searches positions starting at `num_input_tokens`, inclusive."""
        # Sequence layout: prompt = [1, 2] (length 2), first generated token (index 2) is the stop.
        output_ids = torch.tensor([[1, 2, 99, 0, 0]], dtype=torch.long)
