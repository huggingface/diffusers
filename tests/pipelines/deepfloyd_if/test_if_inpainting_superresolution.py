...
from diffusers import DDPMScheduler, IFInpaintingSuperResolutionPipeline, UNet2DConditionModel
...
class IFInpaintingSuperResolutionPipelineTesterConfig(BasePipelineTesterConfig):
    ...
    batch_input_params = frozenset(["prompt", "image", "mask_image", "negative_prompt", "original_image"])
    ...
    def get_dummy_inputs(self):
        image = floats_tensor((1, 3, 16, 16), rng=random.Random(0)).to(torch_device)
        original_image = floats_tensor((1, 3, 32, 32), rng=random.Random(0)).to(torch_device)
...
@skip_mps
class TestIFInpaintingSuperResolutionPipeline(IFInpaintingSuperResolutionPipelineTesterConfig, PipelineTesterMixin):
    def test_inference(self):
        ...
        # fmt: off
        expected_slice = torch.tensor([0.8358, 0.7508, 0.4206, 0.2589, 0.5189, 0.4049, 0.7819, 0.3033, 0.2914, 0.7931, 0.3436, 0.1623, 0.9617, 0.7002, 0.7946, 0.4972])
        # fmt: on
...
@skip_mps
class TestIFInpaintingSuperResolutionPipelineMemory(
    IFInpaintingSuperResolutionPipelineTesterConfig, PipelineOffloadTesterMixin
):
    pass
...
@slow
@require_torch_accelerator
class TestIFInpaintingSuperResolutionPipelineSlow:
