...
class IPAdapterTesterMixin(BasePipelineOutputMixin):
    ...
    `PipelineTesterMixin` one. Pipelines whose IP-Adapter API differs use a mixin of their own; see
    `FluxIPAdapterTesterMixin` below.
    """
...
class FluxIPAdapterTesterMixin(BasePipelineOutputMixin):
    """IP-Adapter tests shared by the Flux pipelines and by the other pipelines built on the Flux IP-Adapter
    API (Chroma, Flux ControlNet).
