...
class ResnetBlock2DTests(unittest.TestCase):
    ...
    def test_resnet_up(self):
        ...
        output_slice = output_tensor[0, -1, -3:, -3:]
        expected_slice = torch.tensor([1.2130, -0.8753, -0.9027, 1.5783, -0.5362, -0.5001, 1.0726, -0.7732, -0.4182])
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
    ...
    def test_resnet_down(self):
        ...
        output_slice = output_tensor[0, -1, -3:, -3:]
        expected_slice = torch.tensor([-0.3002, -0.7135, 0.1359, 0.0561, -0.7935, 0.0113, -0.1766, -0.6714, -0.0436])
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
    ...
    def test_restnet_with_kernel_fir(self):
        ...
        assert output_tensor.shape == (1, 32, 32, 32)
        output_slice = output_tensor[0, -1, -3:, -3:]
        expected_slice = torch.tensor([-0.0934, -0.5729, 0.0909, -0.2710, -0.5044, 0.0243, -0.0665, -0.5267, -0.3136])
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
