...
class Transformer2DModelTests(unittest.TestCase):
    ...
    def test_spatial_transformer_cross_attention_dim(self):
        ...
        output_slice = attention_scores[0, -1, -3:, -3:]
        expected_slice = torch.tensor([0.0143, -0.6909, -2.1547, -1.8893, 1.4097, 0.1359, -0.2521, -1.3359, 0.2598], device=torch_device)
        assert torch.allclose(output_slice.flatten(), expected_slice, atol=1e-3)
    ...
    def test_spatial_transformer_timestep(self):
        ...
        expected_slice = torch.tensor([-0.3923, -1.0923, -1.7144, -1.5570, 1.4154, 0.1738, -0.1157, -1.2998, -0.1703], device=torch_device)
