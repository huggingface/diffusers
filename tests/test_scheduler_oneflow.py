...
from diffusers import OneFlowEulerDiscreteScheduler as EulerDiscreteScheduler
from diffusers import OneFlowEulerAncestralDiscreteScheduler as EulerAncestralDiscreteScheduler
from diffusers import OneFlowHeunDiscreteScheduler as HeunDiscreteScheduler
from diffusers import OneFlowIPNDMScheduler as IPNDMScheduler
from diffusers import OneFlowLMSDiscreteScheduler as LMSDiscreteScheduler
from diffusers import OneFlowScoreSdeVeScheduler as ScoreSdeVeScheduler
from diffusers import OneFlowVQDiffusionScheduler as VQDiffusionScheduler

from diffusers.configuration_utils import ConfigMixin, register_to_config
from diffusers.schedulers.scheduling_oneflow_utils import OneFlowSchedulerMixin as SchedulerMixin
...
class DDPMSchedulerTest(SchedulerCommonTest):
    ...
    def test_full_loop_no_noise(self):
        ...
        assert abs(result_sum.item() - 267.9130) < 1e-2
        assert abs(result_mean.item() - 0.3488) < 1e-3
    ...
    def test_full_loop_with_v_prediction(self):
        ...
        assert abs(result_sum.item() - 210.6344) < 1e-2
        assert abs(result_mean.item() - 0.2743) < 1e-3
...
class DPMSolverMultistepSchedulerTest(SchedulerCommonTest):
    ...
    def check_over_configs(self, time_step=0, batch1_sample=None, **config):
        kwargs = dict(self.forward_default_kwargs)
        num_inference_steps = kwargs.pop("num_inference_steps", None)
        sample = self.dummy_sample if batch1_sample is None else batch1_sample
        residual = 0.1 * sample
    ...
    def test_thresholding(self):
        # TODO(Liang Depeng): support batch_size > 1
        batch_size = 1
        num_channels = 3
        height = 8
        width = 8
        sample = torch.rand((batch_size, num_channels, height, width))
        self.check_over_configs(batch1_sample=sample, thresholding=False)
        for order in [1, 2, 3]:
            for solver_type in ["midpoint", "heun"]:
                for threshold in [0.5, 1.0, 2.0]:
                    for prediction_type in ["epsilon", "sample"]:
                        self.check_over_configs(
                            batch1_sample=sample,
                            thresholding=True,
...
class ScoreSdeVeSchedulerTest(unittest.TestCase):
    ...
    def test_full_loop_no_noise(self):
        ...
        assert np.isclose(result_sum.item(), 13807350784.0)
        assert np.isclose(result_mean.item(), 17978322.0)
...
class EulerAncestralDiscreteSchedulerTest(SchedulerCommonTest):
    ...
    def test_full_loop_no_noise(self):
        ...
        if torch_device in ["cpu", "mps"]:
            assert abs(result_sum.item() - 147.9534) < 1e-2
            assert abs(result_mean.item() - 0.1926) < 1e-3
        else:
            # CUDA
            assert abs(result_sum.item() - 155.0745) < 1e-2
            assert abs(result_mean.item() - 0.2019) < 1e-3
    ...
    def test_full_loop_with_v_prediction(self):
        ...
        if torch_device in ["cpu", "mps"]:
            assert abs(result_sum.item() - 101.1833) < 1e-2
            assert abs(result_mean.item() - 0.1317) < 1e-3
        else:
            # CUDA
            assert abs(result_sum.item() - 101.7326) < 1e-2
            assert abs(result_mean.item() - 0.1325) < 1e-3
