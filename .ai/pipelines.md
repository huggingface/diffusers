...
6. **Be deliberate about methods on the pipeline.** `__call__` is the user's mental model of the pipeline, and the methods on the class are how they navigate it. Diffusers convention (flux, sdxl, wan, qwenimage) is a flat class body of public lifecycle methods (`__init__`, `check_inputs`, `encode_prompt`, `prepare_latents`, `__call__`), each invoked from `__call__`. Two principles, not strict rules — use judgment:
